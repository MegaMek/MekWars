...
public class CH extends Command {
     ...
     * @param client
     */
    public CH(client.MWClient mwclient) {
        super(mwclient);
    }
    ...
    public void execute(String input) {
        ...
        if (ST.hasMoreTokens()) {
            ...
            if (nextString.startsWith("(Housemail)") || nextString.startsWith("(Moderator Mail)")) {
                ...
                if (!mwclient.isIgnored(name, client.MWClient.IGNORE_HOUSE) || isModMail) {
                    ...
                    if (isModMail) {
                        mwclient.addToChat(message, client.gui.CCommPanel.CHANNEL_MOD);
                        // also add to main, if configured to do so
                        if (mwclient.getConfig().isParam("MAINCHANNELMM")) {
                            mwclient.addToChat("<font color=\"red\" size=\"" +
                                                     fontSize +
                                                     "\"><b>Mod Mail: </b></font>" +
                                                     message);
                        }
                        ...
                        mwclient.addToChat(message, client.gui.CCommPanel.CHANNEL_HMAIL);
            ...
            // else, this is a system message
                ...
                message = mwclient.getShortTime() + message;
                mwclient.addToChat(message, client.gui.CCommPanel.CHANNEL_ERROR);
            }// end ErrorLog
                ...
                if (!mwclient.isIgnored(name, client.MWClient.IGNORE_PUBLIC)) {
                    ...
                    mwclient.addToChat(message, client.gui.CCommPanel.CHANNEL_RPG);
             ...
             */
                ...
                if (!mwclient.isIgnored(name, client.MWClient.IGNORE_PUBLIC)) {
                    ...
                    if (user.isInvis() &&
                              (user.getUserlevel() > mwclient.getUser(mwclient.getUsername()).getUserlevel())) {
                        isInvisible = true;
                    } else {
                        isInvisible = false;
                    }
            ...
            if (!wasSystemMessage || checkSysMessages) {
                ...
                if (wasICMessage &&
                    ...
                    mwclient.addToChat(message, client.gui.CCommPanel.CHANNEL_PLOG);// log the message
                    ...
                    // keep logging, even if sound is disabled
                    mwclient.addToChat(message, client.gui.CCommPanel.CHANNEL_PLOG);
                }
            }
