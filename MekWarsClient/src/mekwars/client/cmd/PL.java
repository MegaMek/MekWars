...
public class PL extends Command {
     ...
     * @param client
     */
    public PL(client.MWClient mwclient) {
        super(mwclient);
    }
    ...
    public void execute(String input) {
        ...
        if (cmd.equals("RA")) {
            ...
            client.campaign.CUnit unit = new client.campaign.CUnit(mwclient);

            unit.setUnitFilename(filename);
            unit.createEntity();
            unit.setPilot(new Pilot("Jeeves", 4, 5));
            UnitUtils.applyBattleDamage(unit.getEntity(), damage, true);
            new client.gui.dialog.AdvancedRepairDialog(mwclient, unit, unit.getEntity(), false);
            ...
            player.getMyHouse()
                  .setNonFactionUnitsCostMore(Boolean.parseBoolean(mwclient.getserverConfigs(
            ...
            try {
                StringBuilder userData = new StringBuilder(client.MWClient.CAMPAIGN_PREFIX + "c sendclientdata#");
            ...
            mwclient.doParseDataInput("CH|AM: Targeting for unit " +
                                            unitID +
                                            " set to " +
        ...
        mwclient.refreshGUI(client.MWClient.REFRESH_PLAYERPANEL);
