 ...
 public class HS extends Command {
      ...
      */
     public HS(client.MWClient mwclient) {
         super(mwclient);
     }
     ...
     /**
      * HS| Commands can be issued in bulk. This allows short ops, etc to send ALL changes they make ot impacted house
      * players at once instead of sending 5-10 separate updates.
      */
     private void issueSubCommand(String cmdName, String cmdData) {
         ...
         {mwclient.getMainFrame().getMainPanel().getHSPanel().removeFactionFactory(cmdData);} else if (cmdName.equals(
               "CF")) //change a factory, HS|CF
         {mwclient.getMainFrame().getMainPanel().getHSPanel().changeFactionFactory(cmdData);} else if (cmdName.equals(
               "CA"))//clear all HS data. usually used by defect.
