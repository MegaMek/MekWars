...
public class DataFetchClient {
    ...
    public DataFetchClient(int dataport, int socketDelayTime) {
        ...
        if (socketDelayTime > 0) {this.socketDelayTime = socketDelayTime;} else {this.socketDelayTime = 2000;}
    ...
    public void checkServerVersion(MWDedHost dedHost) throws IOException {
        ...
        if (!mustUpdate) {
            ...
            if (forceUpdateKey.trim().length() > 1) {mustUpdate = !forceUpdateKey.equals(clientUpdateKey);}
        }
        ...
        try {
            ...
            try {
                while (true) {
                    ...
                    FileOutputStream fops = new FileOutputStream(cacheDir +
                                                                       "/" +
                                                                       faction.toLowerCase() +
                                                                       "traitnames.txt");
                    PrintStream out = new PrintStream(fops);
    ...
    private BinReader openConnection(String cmd, int timeout) throws IOException {
        ...
        out.println(cmd, "cmd");
        if (lastTimestamp == null) {out.println("", "lasttimestamp");} else {
