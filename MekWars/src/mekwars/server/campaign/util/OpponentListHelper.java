    ...
    private void getOpponentsForAll() {
        ...
        for (House h : CampaignMain.cm.getData().getAllHouses()) {
            ...
            for (SPlayer currPlayer : currHouse.getActivePlayers().values()) {
                ...
                if (CampaignMain.cm.getIThread().isImmune(currPlayer)) {continue playersLoop;}

                //if player has no armies, skip him
                if (currPlayer.getArmies().size() == 0) {continue playersLoop;}

                //check exclusion status of the players.
                if (currPlayer.getExclusionList().checkExclude(searchPlayer.getName()) != server.campaign.util.ExclusionList.NO_EXCLUSION) {
                    continue playersLoop;
                }
                if (searchPlayer.getExclusionList().checkExclude(currPlayer.getName()) != server.campaign.util.ExclusionList.NO_EXCLUSION) {
                    continue playersLoop;
                }
