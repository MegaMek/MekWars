...
public class AttackCommand implements Command {
    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (ap == null) {
            CampaignMain.cm.toUser("AM:Null player. Contact an administrator to report this, immediately!",
                  Username,
                  true);
            return;
        }
        ...
        if (altID >= 0) {
            CampaignMain.cm.toUser("AM:You're only allowed to attack once, and are already in Attack #" + altID + ".",
                  Username,
                  true);
            return;
        }
        ...
        if (!minActiveMet) {
            CampaignMain.cm.toUser("AM:You're still on your way to the frontline. You cannot attack until you arrive.",
                  Username,
                  true);
            return;
        }
        ...
        if (aa == null) {
            ...
            CampaignMain.cm.toUser("AM:Army #" + armyID + " has a BV of 0 and may not be used to attack.",
                  Username,
                  true);
            return;
        } else if (aa.isDisabled()) {
            CampaignMain.cm.toUser("AM: Army #" + armyID + " is disabled and may not be used to attack.",
                  Username,
                  true);
            return;
        }
        ...
        if (o.getTypeIndicator() == Operation.TYPE_SHORTANDLONG) {
            ...
            if (!manager.hasSpecificLongOnPlanet(ap.getHouseFightingFor(), target, o)) {
                CampaignMain.cm.toUser("AM:Your faction has no " + opName + " in progress on " + target.getName() + ".",
                      Username,
                      true);
                return;
            }
