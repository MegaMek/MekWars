    ...
    public void process(StringTokenizer command, String Username) {
        ...
        if (accessLevel != 0) {
            ...
            if (userLevel < getExecutionLevel()) {
                CampaignMain.cm.toUser("AM:Insufficient access level for command. Level: " +
                                             userLevel +
                                             ". Required: " +
                                             accessLevel +
                                             ".", Username, true);
        ...
        if (dp == null) {
            CampaignMain.cm.toUser("AM:Null player in AcceptAttackFromReserve. Report this immediately!",
                  Username,
                  true);
            return;
        }
        ...
        if (aa == null) {
            CampaignMain.cm.toUser("AM:Defend failed. Attacker does not have an army with ID #" + armyID + ".",
                  Username,
                  true);
            return;
        }
        ...
        if (da.isDisabled()) {
            CampaignMain.cm.toUser("AM:Defend failed. Army #" + armyID + " is disabled and cannot be used to defend.",
                  Username,
                  true);
            return;
        }
        ...
        if ((CampaignMain.cm.getIntegerConfig("MaxNegativeBaysForAFR") > -1) &&
                  ((dp.getFreeBays() + CampaignMain.cm.getIntegerConfig("MaxNegativeBaysForAFR")) < 0)) {
            CampaignMain.cm.toUser("AM:Defend failed. " + dp.getName() + " has too many negative bays.",
                  ap.getName(),
                  true);
        ...
        if (launchTime + (Long.parseLong(CampaignMain.cm.getConfig("AttackFromReserveResponseTime")) * 60000) <
        ...
        if (defensiveFailures.size() == 0) {
            ...
            CampaignMain.cm.toUser("AM:Army #" +
                                         da.getID() +
                                         "could not defend " +
