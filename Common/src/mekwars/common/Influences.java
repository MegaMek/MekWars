...
public class Influences implements common.MutableSerializable {
    ...
    public int getInfluence(int factionID) {
        ...
        if (!influences.containsKey(factionID)) {return 0;}
    ...
    public Integer getOwner() {
        ...
        try {
            TreeSet<House> sset = new TreeSet<House>(new Comparator<Object>() {
                public int compare(Object o1, Object o2) {
                    try {
                        ...
                        if (o1 != null) {i1 = ((House) o1).getId();}

                        if (o2 != null) {i2 = ((House) o2).getId();}
            ...
            int i = 0;
            for (Iterator<House> it = sset.iterator(); it.hasNext(); ) {factions[i++] = it.next();}
            ...
            if (factions.length <= 1) {return faction.getId();}
    ...
    public Influences difference(Influences infNew) {
        ...
        for (Iterator<House> it = other.iterator(); it.hasNext(); ) {
            House h = it.next();
            if (!thisone.contains(h.getId())) {diff.put(h.getId(), (-infNew.getInfluence(h.getId())));}
        }
    ...
    public void add(Influences infNew) {
        ...
        for (Iterator<House> it = infNew.getHouses().iterator(); it.hasNext(); ) {
            House h = it.next();
            if (!getHouses().contains(h)) {influences.put((h.getId()), (infNew.getInfluence(h.getId())));}
        }
        for (Iterator<House> it = getHouses().iterator(); it.hasNext(); ) {
            House h = it.next();
     ...
     *       <p>
     *       public void binOut(TreeWriter out) { Object h[] = influences.keySet().toArray(); Arrays.sort(h, new
     *       Comparator(){ public int compare(Object o1, Object o2) { int i1 = ((Integer)o1).intValue(); int i2 =
     *       ((Integer)o2).intValue(); return i1 == i2 ? 0 : (i1<i2?-1:1); }}); out.write(h.length, "size"); for (int i
     *       = 0; i < h.length; i++) { out.write(((Integer)h[i]).intValue(), "faction");
     ...
     *       <p>
     *       public void binIn(TreeReader in, CampaignData dataProvider) throws IOException { influences = new
     *       HashMap(); int size = in.readInt("size"); for (int i = 0; i < size; i++) { int hid = in.readInt("faction");
     *       int flu = in.readInt("amount"); influences.put((hid),(flu)); } }
     */
