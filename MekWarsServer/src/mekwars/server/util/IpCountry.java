...
public class IpCountry {
    ...
    public IpCountry(String fileIps, String fileNames) {
        this();
        this.file = new mekwars.server.util.FileOps(fileIps);
    ...
    public String seachIpCountry(String ipSource) {
        try {
            ...
            while (!file.eofFile() && //end of file of search so rare but possible.
                         mekwars.server.util.BoolMatrixOps.GreaterOrEqual(ip,
                               ipTable) //ip < ipTable such the file is SHORTED no need to continue the search
            ) {
                cad = file.getTextFromFile();
                ipTable = getIp(cad);
                mask = getMask(cad);
                thisOne = mekwars.server.util.BoolMatrixOps.equal(mekwars.server.util.BoolMatrixOps.and(ip, mask),
                      ipTable);
                found |= thisOne;
                if (thisOne) {
    ...
    private String largeName(String shortName, mekwars.server.util.FileOps file) {
