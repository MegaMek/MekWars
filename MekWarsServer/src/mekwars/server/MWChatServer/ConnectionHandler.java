 ...
 * <p>
 * As each line is read from the socket, the server is notified, via the IConnectionListener interface
 * <p>
 * Outgoing messages are passed by ChatServer to the Dispatcher who queues them up and calls flush on the different CHes
 * in turn.
 */
public class ConnectionHandler extends AbstractConnectionHandler {
    ...
    protected java.io.InputStream _inputStream = null;
     ...
     */
    public ConnectionHandler(java.net.Socket socket, MWChatClient client) throws java.io.IOException {
        ...
        _out = new java.io.PrintWriter(new java.io.OutputStreamWriter(_socket.getOutputStream(), "UTF8"));
        _inputStream = socket.getInputStream();
    }

    /**
     * Called from MWChatClient. Start reading incoming chat and sending to an associated dispatcher.
     */
    void init() {
    ...
    /**
     * @param notify to notify the ConnectionListener. Should be true for unexpected shutdowns (like if there is a
     *               socket error), and false otherwise (if client called this method on purpose)
     */
    @Override
