...
public final class SPlayer extends Player implements Comparable<Object>, IBuyer, ISeller {
    ...
    private static final MMLogger LOGGER = MMLogger.create(SPlayer.class);
    //@salient Mini Campaign Phases
    private static final String RESTOCK_MC = "restock_mc"; //@salient for mini campaigns
    private static final String ACTIVE_MC = "active_mc"; //@salient for mini campaigns
    private final Vector<Integer> totalTechs = new Vector<>(4, 1);
    private final Vector<Integer> availableTechs = new Vector<>(4, 1);
    private final SPersonalPilotQueues personalPilotQueue = new SPersonalPilotQueues();
    ...
    /**
     * Stock constructor. Note that an SPlayer is data-less unless/until fromString() or some sets are called. SPlayers
     * are created in only two places - CampaignMain's load method and the EnrollCommand.
     */
    public SPlayer() {
        ...
        myHouse = CampaignMain.campaignMain.getHouseFromPartialString(CampaignMain.campaignMain.getConfig(
              "NewbieHouseName"));
    ...
    public boolean equals(Object object) {
        if (object instanceof SPlayer otherPlayer) {
    ...
    /**
     * Determine whether a player can use a unit of a given weight class. This is used to prevent new players from
     * buying heavier/larger units and sucking a house dry.
     *
     * @param weightClass weight class to check.
     */
    public boolean mayUse(int weightClass) {
        //@Salient adding this in for Gunny
        if (weightClass == Unit.LIGHT) {
            if (MathUtility.parseInt(getMyHouse().getConfig("MinEXPForLight"), Integer.MAX_VALUE) > experience) {
                return false;
            }
        }
        if (weightClass == Unit.MEDIUM) {
            if (MathUtility.parseInt(getMyHouse().getConfig("MinEXPForMedium"), Integer.MAX_VALUE) > experience) {
                return false;
            }
        }
        if (weightClass == Unit.HEAVY) {
            if (MathUtility.parseInt(getMyHouse().getConfig("MinEXPForHeavy"), Integer.MAX_VALUE) > experience) {
                return false;
            }
        }
        if (weightClass == Unit.ASSAULT) {
            return MathUtility.parseInt(getMyHouse().getConfig("MinEXPForAssault"), Integer.MAX_VALUE) <= experience;
        }
     ...
     */
    public void setMyHouse(SHouse sHouse) {
        myHouse = sHouse;
        setSave();
    }

    /**
     * Add a unit to the player. Pass-though to addUnit(SUnit,boolean,boolean). This version should be called in almost
     * all situations.
     */
    public void addUnit(SUnit sUnit, boolean isNew) {
        this.addUnit(sUnit, isNew, true);
    }

    /**
     * ISeller-compliant .removeUnit(). Simply get the unit ID and pass to normal SPlayer.removeUnit(int,bool). Use the
     * (int,boolean) version of remove unit whenever possible in order to intelligently pass select the army update
     * option. ISeller assumes true and sends updates to all armies.
     *
     * @urgru 1.2.06
     */
    public String removeUnit(SUnit unitToRemove, boolean sendHouseStatusUpdate) {
        ...
        CampaignMain.campaignMain.toUser("PL|SHP|" + penaltyString, name, false);
        return "";// dummy string returned for IBuyer
    }

    /**
     * Return an SUnit with a given unique ID. If the player doesn't own the unit, return a null.
     *
     * @param id - id the unit to return
     *
    ...
    /**
     * Calculates and returns the string to be sent to the client to set both the maintenance penalty and the purchase
     * price penalty for each unit type and weight.
     *
     */
    public String buildHangarPenaltyString() {
        StringBuilder toReturn = new StringBuilder();

        toReturn.append(calculateTotalHangarPenalty());

        for (int type = Unit.MEK; type < Unit.MAX_BUILD; type++) {
            for (int weight = Unit.LIGHT; weight <= Unit.ASSAULT; weight++) {
                toReturn.append("*").append(calculateHangarPenaltyForNextPurchase(type, weight));
            }
        }
    ...
    public int calculateHangarPenaltyForNextPurchase(int type, int weight) {
        ...
        int limit = CampaignMain.campaignMain.getHouseFromPartialString(getMyHouse().getName())
        ...
        penalty = (int) (Math.pow(penaltyUnits,
    ...
    /**
     * A method to determine if the player is over the unit limit and the server is configured to use sliding hangar
     * cost increases
     *
     *
     */
    ...
    /**
     * Remove the Unit with ID unitid from the player. Ops are checked by discrete commands (ie - SellUnit), unchecked
     * by large blocks of code which force a check on their own (ie - ShortResolver).
     *
    ...
    public void removeUnit(int unitid, boolean sendArmyUpdate) {
        ...
        for (SArmy currA : getArmies()) {
            if (currA.getUnitPosition(unitid) > -1) {
                ...
                if (sendArmyUpdate) {
                    CampaignMain.campaignMain.toUser("PL|SAD|" + currA.toString(true, "%"), name, false);
                    CampaignMain.campaignMain.getOpsManager().checkOperations(currA, true);// update
                    // legal
                    // ops
                }
            }
        ...
        CampaignMain.campaignMain.toUser("PL|RU|" + unitid, name, false);
    ...
    /**
     * This method does all the math to figure out how much the retainer fee, maintenance cost, whathaveyou is for the
     * current number of technicians. The number itself is useful in some cases (let people know what they will have to
     * pay after hiring a new tech, for example), and thus separated from the actual payment. For now, we have only one
     * payment calculation mechanism -- additive costing, whereby each tech costs as much as the last, plus a constant
     * kicker. A cap to this cost can be configured; however, it must be a multiple of the per-tech additive (eg, if the
    ...
    private void doPayTechniciansMath() {
        ...
        for (int type_id = Unit.MEK; type_id < Unit.MAX_BUILD; type_id++) {
    ...
    /**
     * Should be called only after an attempt to pay techs comes up short. At present, only used by ShortResolver. Other
     * times techs are paid (eg - TransferCommand) shortfalls stop the player from acting. Does all the dirty work of
     * lowering the number of technicians and setting units as unmaintained.
     *
     * @param amountOfShortFall - the amount owed to techs which can't be paid. used to determine how many walk off /
     *                          quit.
     *
    ...
    /**
     * Method that returns the current cost of hiring a new technician, after adjustment for XP, etc. Used by
     * HireTechsCommand, Requests and SetMaintainedCommand.
     */
    public int getTechHiringFee() {
        // get the starting tech cost
        int techCost = Integer.parseInt(CampaignMain.campaignMain.getConfig("BaseTechCost"));
    ...
    /**
     * A method which is called to randomly set some units as unmaintained when support levels go negative. Continues
     * until support number is positive again, or all units are unsupported (catches odd problems with units on the
     * black market -- not an expecially graceful solution; however, the alternative is allowing units on the BM to be
     * scrapped mid-auction).
     *
     * @urgru 8/2/04
     */
    public int setRandomUnmaintained() {
        ...
        // destroyed
        java.util.Vector<mekwars.server.campaign.SUnit> okUnitsData = new java.util.Vector<mekwars.server.campaign.SUnit>(
              1,
              1);
        for (SUnit currU : units) {
        ...
        while (getFreeBays() < 0) {
            ...
            int rnd = CampaignMain.campaignMain.getRandomNumber(okUnitsData.size());// generate
            // a
            // RND
            SUnit unit = okUnitsData.elementAt(rnd);// get unit @ rnd location
            unit.setUnmaintainedStatus();// make it unmaintained
            numUnmaintained++;
            CampaignMain.campaignMain.toUser("PL|UU|" + unit.getId() + "|" + unit.toString(true), name, false);
    ...
    public int getFreeBays() {
        ...
        int totalProtos = 0;
        boolean advanceRep = CampaignMain.campaignMain.isUsingAdvanceRepair();
        ...
        for (SUnit currU : units) {
            ...
            if (((currU.getStatus() == Unit.STATUS_OK) || (currU.getStatus() == Unit.STATUS_FOR_SALE)) &&
         ...
         */
        if (totalProtos > 0) {
            int techRatio = Integer.parseInt(CampaignMain.campaignMain.getConfig("TechsToProtoPointRatio"));
    ...
    public void setSave() {
        if (!isLoading) {
            CampaignMain.campaignMain.forceSavePlayer(this);
        }
    }

    /**
     * This can be calcualted in one of three "standard" ways: 1) House bays + techs 2) House bays + experience 3) House
     * bays + techs + experience Or, two additional ways if using Advanced Repair: 4) House Bays + bays owned by player
     * 5) House bays + bays owned by player + experience
     *
    ...
    public int getTotalMekBays() {// return bay/support number
        ...
        boolean usesAdvanceRepairs = CampaignMain.campaignMain.isUsingAdvanceRepair();
    ...
    public void setBaysOwned(int bays) {
        ...
        if (getMyHouse() != null) {
            ...
            maxBays = CampaignMain.campaignMain.getIntegerConfig("MaxBaysToBuy");
        }
    ...
    /**
     * Loop through the units and perform maintainance. Check status and adjust maintainance level accordingly. This is
     * called during slices. Check to ses if units are maintained -- if so, improve maintainance levels. If not, roll a
     * random. If its greater than the maintainance level, scrap the unit. If unit should be scrapped, or just have its
     * mainainance level reduced. Note that units on the BM arent included in the maintainance loop. It should be
     * impossible to add an unmaintained unit to the BM, but just in case, they're excluded (STATUS_FORSALE is ignored).
     * This prevents off BM nulls.
     */
    public void doMaintainance() {
        ...
        java.util.ArrayList<mekwars.server.campaign.SUnit> unitsToDestroy = new java.util.ArrayList<mekwars.server.campaign.SUnit>();
        for (SUnit currUnit : units) {// loops through all units
            ...
            if (currUnit.getStatus() == Unit.STATUS_OK) {
                currUnit.addToMaintenanceLevel(increase);
                ...
                if (CampaignMain.campaignMain.getImmunityThread().isImmune(this) ||
                    ...
                    currUnit.addToMaintenanceLevel(-decrease);
                } else if (rnd <= currUnit.getMaintenanceLevel()) {
                    currUnit.addToMaintenanceLevel(-decrease);
                }
                ...
                // unmaintained and failed scrap check. blow 'er up.
                    ...
                    // amount.
                    int mechscrapprice = Math.round(myHouse.getPriceForUnit(currUnit.getWeightClass(),
                          currUnit.getType()) * Float.parseFloat(getMyHouse().getConfig("ScrapCostMultiplier")));
                    ...
                    CampaignMain.campaignMain.toUser(toSend, name, true);
    ...
    public void setActive(boolean newStatus) {
        ...
        if (!newStatus) {
            ...
            /*
             * Player is being moved to inactive status. This means he is no
             * longer an eligible attack target. Need to remove his op lists and
             * clear his entries on other players op lists.
             */
            OpponentListHelper opponentListHelper = new OpponentListHelper(this, OpponentListHelper.MODE_REMOVE);
            opponentListHelper.sendInfoToOpponents("left the front lines and may no longer be attacked");
             ...
             * against the player.
             */
            CampaignMain.campaignMain.getOpsManager().removePlayerFromAllPossibleDefenderLists(name, true);
             ...
             * hit 0 attackers.
             */
            CampaignMain.campaignMain.getOpsManager().removePlayerFromAllAttackerLists(this, null, true);
    ...
    public void setRating(double d) {
        ...
        if (!Boolean.parseBoolean(getMyHouse().getConfig("HideELO"))) {
            ...
            CampaignMain.campaignMain.toUser("PL|SR|" + rounded, name, false);
            CampaignMain.campaignMain.doSendToAllOnlinePlayers("PI|RA|" + name + "|" + rounded, false);
        }
    ...
    public double getRatingRounded() {
        java.math.BigDecimal bd = new java.math.BigDecimal(rating);
    ...
    /**
     * Method which sets a player to fighting without triggering Oplist construction. DO NOT USE THIS METHOD. It is a
     * special activation/business sequence that is used only when a player is returning to the server and already
     * involved in a game and should only be called from ShortOperation. All standard activations and ALL deactivations
     * should be dealt with via SPlayer.setActive(boolean), which sets up opponent lists, informs potential attackers,
     * etc.
     */
     ...
     * Weight for an army is reduced if its BV +/- MaxBVDifference (from campaign configuration) overlaps another armies
     * BV, falls below MinCount or rises above MaxCount. In short, only the portions of an army which may be *uniquely*
     * targetted by opposing forces with the Min/Max range count fully. The weight is automatically reduced by the level
     * of overlap, and server operators may declare additional overlap penalties. Example: Player A has Armies of 3000
     * and 3050 BV. MaxBVDifference is 150, and an OverlapPenalty of .20 is set in campaignconfig.txt - Starting weight
     * is 2 for two armies, - Raw amount of overlap is (150-(3050-3000 = 50))/150 = .67 - Weight after raw overlap
     * adjustment is 2.0 - 0.67 = 1.33 - OverlapPenalty is applied (1.33 - .20 = 1.13) In this case, the final weighted
     * number of armies is 1.37.
     *
    ...
    public double getWeightedArmyNumber() {
        ...
        if (weightedArmyNumber <= 0) {
            ...
            java.util.Vector<mekwars.server.campaign.SArmy> orderedArmies = new java.util.Vector<mekwars.server.campaign.SArmy>(
                  1,
                  1);
            ...
            for (SArmy currentArmy : getArmies()) {
                ...
                if (orderedArmies.size() == 0) {
                    ...
                    java.util.Enumeration<mekwars.server.campaign.SArmy> f = orderedArmies.elements();
            ...
            if (weightedArmyNumber > 0) {
                ...
                java.util.Enumeration<mekwars.server.campaign.SArmy> e = orderedArmies.elements();
    ...
    public java.util.Vector<mekwars.server.campaign.SArmy> getArmies() {
        return armies;
    }
     ...
     * (getWeightedArmyNumber) is called. Should be triggered by anything which changes army BV or army numbers - game
     * resolution and EXM, etc.
     *
     * @urgru 11/12/04
     */
    ...
    public void reset(String confirm) {
        ...
        groupAllowance = 0;
        influence = 0;
        myHouse = CampaignMain.campaignMain.getHouseFromPartialString(getMyHouse().getConfig("NewbieHouseName"), null);
        myLogo = " ";
    ...
    /**
     * A Method to get the current duty status of a player. Options are, from lowest to highest, STATUS_LOGGED_OUT,
     * STATUS_RESERVE, STATUS_ACTIVE, and STATUS_FIGHTING.
     */
    public int getDutyStatus() {
        ...
        if (myHouse.getFightingPlayers().containsKey(lowerName)) {
            return mekwars.server.campaign.SPlayer.STATUS_FIGHTING;
        }
        ...
        if (myHouse.getReservePlayers().containsKey(lowerName)) {
            return mekwars.server.campaign.SPlayer.STATUS_RESERVE;
        }

        // Not in any faction hash. he's logged out.
        return mekwars.server.campaign.SPlayer.STATUS_LOGGED_OUT;
    }

    /**
     * Determines the weighted number of votes a player can cast. Draws a flat config out of campaignconfig.txt to use
     * as a base number. Additional votes may be assigned as a player gains XP, up to a configurable ceiling. Used by
     * the various vote cmds to block over voting, etc.
     *
     ...
     *
     * @param sendStatus - boolean. if true, send the player's status downstream. should usually be true. false when
     *                   called from NewbieHouse, which send status on its own after granting new units.
     */
    public void stripOfAllUnits(boolean sendStatus) {
        units = new java.util.Vector<mekwars.server.campaign.SUnit>(1, 1);
        armies = new java.util.Vector<mekwars.server.campaign.SArmy>(1, 1);

        if (sendStatus) {
            CampaignMain.campaignMain.toUser("PS|" + this.toString(true), name, false);
        }

        setSave();
        CampaignMain.campaignMain.toUser("PL|SHP|" + buildHangarPenaltyString(), name, false);
    }
    ...
    public void addExperience(int i, boolean modAdded) {
        ...
        CampaignMain.campaignMain.toUser("PL|SE|" + experience, name, false);
        CampaignMain.campaignMain.doSendToAllOnlinePlayers("PI|EX|" + name + "|" + experience, false);
        ...
        if (!modAdded && (i > 0)) {
            ...
            if ((currentXP >= rollOver) && (rollOver > 0)) {
                ...
                String toSend = "You earned " +
                                      rpToAdd +
                                      " experience " +
                                      CampaignMain.campaignMain.getConfig("RPShortName");
                toSend += "[<a href=\"MWUSERP\">Use " + CampaignMain.campaignMain.getConfig("RPShortName") + "</a>]";
                CampaignMain.campaignMain.toUser(toSend, name, true);
    ...
    /**
     * Simple private method which returns the next available free position ID (hangar location). While this seems
     * pointless, and probably is, the hangar ID is used by the client for all kinds of things and we're stuck with it
     * until someone takes the time to weed it out completely.
     */
    ...
    /**
     * @return if the player is eligible for welfare light meks from faction bays. due to lack of mechs in bay and they
     *       are all light
     *
    ...
    /**
     * Determine the total BV of all units owned by the player. This is used by the welfare checks to see whether a
     * players units can form an army of sufficient BV. Note that for_sale units are included in the BV total, because
     * skipping them would allow players to list a unit, get welfare units, and then delist the sales unit in order.
     * Freebies is something we want to avoid, because people are evil and cheat.
     *
    ...
    /**
     * @return if enabled, this method will initiate the Restock Phase (currency injection) if hangar is below a certain
     *       threshold. This occurs AFTER a match.
     *
     * @author Salient
     */
    public void checkHangarRestockMC() {
        ...
        if (restock) //the way it's set up, may not need to clear currency since it should be clear already.
        {
            if (restockRP != -1) {
                ...
                addReward(restockRP);
                toSelf("AM: You have received " + getReward() + " " + CampaignMain.campaignMain.getConfig("RPLongName")
            ...
            if (restockCB != -1) {
                ...
                toSelf("AM: You have received " +
                             getMoney() +
                             " " +
                             CampaignMain.campaignMain.getConfig("MoneyLongName")
                             +
                             ". Restock your forces before continuing.");
            }
    ...
    public boolean canActivateForMiniCampaign() {
        ...
        if (isPhaseRestockMC()) {
            ...
            if (restockCB != -1 && leewayCB > 0f && getMoney() > leewayCB) {
                toSelf("AM: You have too many " +
                             CampaignMain.campaignMain.getCurrencyName("money", false) +
                             " to go active!");
                return false;
            }
            ...
            if (restockRP != -1 && leewayRP > 0f && getReward() > leewayRP) {
                toSelf("AM: You have too many " +
                             CampaignMain.campaignMain.getCurrencyName("rp", false) +
                             " to go active!");
                return false;
            }

            if (restockFLU != -1 && leewayFLU > 0f && getInfluence() > leewayFLU) {
                toSelf("AM: You have too much " +
                             CampaignMain.campaignMain.getCurrencyName("flu", false) +
                             " to go active!");
                return false;
            }
    ...
    public void toSelf(String msg) {
        CampaignMain.campaignMain.toUser(msg, getName(), true);
    }
        ...
        for (SUnit currU : units) {
            if (removeLockedBV) // do not add BV of units that are locked.
            {
                if (currU.isLocked() == false) //if unit is locked, ignore it
                {
                    ...
                    {continue;} else {bv += currU.getBVForMatch();}
                }
    ...
    private boolean isPhaseRestockMC() {
        if (phaseMC.equalsIgnoreCase(RESTOCK_MC)) {return true;} else {return false;}
    }
    ...
    public void setReward(int i) {
        ...
        CampaignMain.campaignMain.toUser("PL|SRP|" + rewardPoints, name, false);
        setSave();
    }
    ...
    public void setInfluence(int i) {
        ...
        CampaignMain.campaignMain.toUser("PL|SI|" + influence, name, false);
        setSave();
    }
    ...
    public boolean isAtOrOverUnitLimits() {
        ...
        boolean dontCountAero = CampaignMain.campaignMain.getBooleanConfig("IgnoreAeroUnitLimit");
        ...
        for (int t = Unit.MEK; t <= uType; t++) {
            for (int w = Unit.LIGHT; w <= Unit.ASSAULT; w++) {
                ...
                if (limit != -1) {
                    if (inHangar >= limit) {result = true;} else {return false;}
                }
            }
        }
        return result;
    ...
    public void addMoney(int i) {
        ...
        money = moneyToSet;
        CampaignMain.campaignMain.toUser("PL|SM|" + money, name, false);
        setSave();
    }
    ...
    /**
     * Add a unit to the player. If the unit is new, make it immune to maintenance scraps. Nearly all calls should send
     * updates to a client; however, in some instances (ex: when giving units to a SOL player), bandwidth is saved by
     * doing a single PS| at the end of a series of adds.
     */
    public String addUnit(SUnit sUnit, boolean isNew, boolean sendUpdates) {

        if (isNew) {
            long immunityTime = MathUtility.parseLong(getMyHouse().getConfig("ImmunityTime"), 0) * 1000;
            sUnit.setPassesMaintenanceUntil(System.currentTimeMillis() + immunityTime * 2);
        }

        // clear any scrap allowance
        sUnit.setScrappableFor(-1);
         ...
         */
        if (getFreeBays() <
                  (CampaignMain.campaignMain.isUsingIncreasedTechs() ?
                         SUnit.getHangarSpaceRequired(sUnit,
                               getMyHouse().houseSupportsUnit(sUnit.getUnitFilename()),
                               getMyHouse()) :
                         SUnit.getHangarSpaceRequired(sUnit, getMyHouse()))) {
            sUnit.setUnmaintainedStatus();
        } else {
            sUnit.setStatus(Unit.STATUS_OK);
        }

        // strip illegal ammo
        SUnit.checkAmmoForUnit(sUnit, myHouse);

        //@salient add quirks to entity
        //as seen recently, an improper setup quirks xml can cause a npe here.
         ...
         */
        if (sendUpdates) {
            CampaignMain.campaignMain.toUser("PL|HD|" + sUnit.toString(true), name, false);
            CampaignMain.campaignMain.toUser("PL|SUS|" + sUnit.getId() + "#" + sUnit.getStatus(), name, false);
            CampaignMain.campaignMain.toUser("PL|SB|" + getTotalMekBays(), name, false);
            CampaignMain.campaignMain.toUser("PL|SF|" + getFreeBays(), name, false);
        }
    ...
    public boolean hasMT() {
        if (getRemainingMekTokens() != 0) {return true;} else {return false;}
    }
    ...
    public void addMekToken(int i) {
        ...
        CampaignMain.campaignMain.toUser("PL|UMT|" + tokenToSet, name, false); //UMT: Update Mek Token on cplayer
        setSave();
    ...
    private void loadStatusMC(String data) {
        ...
        if (st.hasMoreTokens()) {phaseMC = TokenReader.readString(st);} else {
    ...
    public void refreshGUI() {
        CampaignMain.campaignMain.toUser("RG|" + " ", name, false);
    }
    ...
    public void addAvailableTechs(int type, int number) {
        ...
        CampaignMain.campaignMain.toUser("PL|UAT|" + availableTechsToString(), name, false);
    ...
    public void addTotalTechs(int type, int number) {
        ...
        CampaignMain.campaignMain.toUser("PL|UTT|" + totalTechsToString(), name, false);
    }
    ...
    public void updateAvailableTechs(String data) {
        try {
            java.util.StringTokenizer techs = new java.util.StringTokenizer(data, "%");
    ...
    /**
     * Method which determines which house a player is actually fighting for. Used to display contracting house, instead
     * of real faction, for mercenaries.
     */
    ...
    public void removeArmy(int armyID) {
        ...
        java.util.Iterator<mekwars.server.campaign.SArmy> i = armies.iterator();
        while (i.hasNext()) {
            SArmy currA = i.next();
        ...
        CampaignMain.campaignMain.toUser("PL|RA|" + armyID, name, false);
    }
    ...
    public int compareTo(Object o) {
        mekwars.server.campaign.SPlayer p = (mekwars.server.campaign.SPlayer) o;
    ...
    public java.util.Date getLastOnlineDate() {
        return new java.util.Date(lastOnline);
    }
    ...
    public int getAmountOfTimesUnitExistsInArmies(int unitID) {
        int result = 0;
        java.util.Vector<mekwars.server.campaign.SArmy> v = getArmies();
    ...
    public void checkAndUpdateArmies(SUnit unit) {
        ...
        for (SArmy army : armies) {
            if (army.isUnitInArmy(unit)) {
                army.setBV(0);
                CampaignMain.campaignMain.toUser("PL|SABV|" + army.getID() + "#" + army.getBV(), name, false);
            }
        }
    ...
    /**
     * Method which returns a boolean indicating whether any units in all the armies or any units period are being
     * repaired.
     *
    ...
    public void healAllPilots() {
        try {
            ...
            // /=60000;
            int tickTime = CampaignMain.campaignMain.getIntegerConfig("TickTime");
        ...
        if (Boolean.parseBoolean(getMyHouse().getConfig("AllowPersonalPilotQueues"))) {
            ...
            for (int type : typeList) {
                for (int weight = 0; weight <= Unit.ASSAULT; weight++) {
                    LinkedList<Pilot> list = personalPilotQueue.getPilotQueue(type, weight);
    ...
    public void healPilots() {
        ...
        if (Boolean.parseBoolean(getMyHouse().getConfig("AllowPersonalPilotQueues"))) {
            ...
            for (int type : typeList) {
                for (int weight = 0; weight <= Unit.ASSAULT; weight++) {
                    java.util.List<Pilot> list = personalPilotQueue.getPilotQueue(type, weight);
                    for (Pilot pilot : list) {
                        if (pilot.getHits() <= 0) {
                            continue;
                        }
    ...
    /**
     * Complete human readable status of a player. Absolutely must be maintained and properly updated at all times. /c
     * mystatus is the best/only way to accurately confirm a client's data representation vs. the player's state
    ...
    public String getReadableStatus(boolean adminStatus) {
        ...
        s.append("  " +
                       CampaignMain.campaignMain.moneyOrFluMessage(true, false, getMoney()) +
                       " //  " +
                       CampaignMain.campaignMain.moneyOrFluMessage(false, false, influence) +
                       " // " +
                       experience +
                       " Experience<br>");
        ...
        if (CampaignMain.campaignMain.isUsingAdvanceRepair()) {
            ...
            s.append("Leased Bays: " +
                           getBaysOwned() +
                           " (Cost: " +
                           CampaignMain.campaignMain.moneyOrFluMessage(true, false, getCurrentTechPayment()) +
                           "/Game)<br>");
        }
        ...
        if (Boolean.parseBoolean(getMyHouse().getConfig("VotingEnabled"))) {
            int votesCast = CampaignMain.campaignMain.getVoteManager().getAllVotesBy(this).size();
        ...
        if ((Integer.parseInt(getMyHouse().getConfig("NoPlayListSize")) > 0) ||
            ...
            java.util.Enumeration<String> en = exclusionList.getPlayerExcludes().elements();
        ...
        for (SUnit currU : units) {
            ...
            if (currU.getStatus() == Unit.STATUS_FOR_SALE) {
                continue;
            }

            if (adminStatus) {
                s.append(currU.getDescription(false)).append("<br>");
            } else {
                s.append(currU.getDescription(true)).append("<br>");
            }
        }
        ...
        for (SUnit currU : units) {
            ...
            if (adminStatus) {
                saleUnits.append(currU.getDescription(false)).append("<br>");
            } else {
                saleUnits.append(currU.getDescription(true)).append("<br>");
            }
        }
        ...
        for (String flag : pFlags.getFlagNames()) {
            s.append(flag).append(": ").append(pFlags.getFlagStatus(flag)).append("<br>");
        }
    ...
    /**
     * Method that returns a human readable string containing special info pertinent to mercenaries, such an employer
     * and contract terms.
     */
    ...
    public String toString(boolean toClient) {
        ...
        if (!toClient) {
            ...
            if (password != null) {
                result.append(password.getAccess().ordinal());
        ...
        if (!toClient) {
            ...
            if (getSubFactionName().trim().isEmpty()) {
                result.append(" ");
            } else {
        ...
        result.append(exportFlags().length() > 1 ?
                            exportFlags() :
                            CampaignMain.campaignMain.getDefaultPlayerFlags().export());
    ...
    public void fromString(String s) {
        ...
        try {
            ...
            for (int i = 0; i < numofUnits; i++) {
                ...
                units.add(m);
                CampaignMain.campaignMain.toUser("PL|HD|" + m.toString(true), name, false);
            }
            ...
            for (int i = 0; i < numofarmies; i++) {
                ...
                CampaignMain.campaignMain.toUser("PL|SAD|" + a.toString(true, "%"), name, false);
            }

            setMyHouse(CampaignMain.campaignMain.getHouseFromPartialString(TokenReader.readString(ST), null));
            ...
            if (CampaignMain.campaignMain.isUsingAdvanceRepair()) {
                ...
                int mt = CampaignMain.campaignMain.getIntegerConfig("MaxTechsToHire");
            ...
            myHouse = CampaignMain.campaignMain.getHouseFromPartialString(TokenReader.readString(ST));
            ...
            try {
                AccessRole access = AccessRole.valueOf(Integer.toString(TokenReader.readInt(ST)));
            ...
            loadFlags(CampaignMain.campaignMain.getDefaultPlayerFlags().export());
            ...
            if (ST.hasMoreTokens()) {
                ...
                if (CampaignMain.campaignMain.getBooleanConfig("Enable_BotPlayerInfo")) {
            ...
            if ((password != null) && (password.getPasswd().trim().length() <= 2)) {
                password.setAccess(AccessRole.GUEST);
            }
            ...
            if (CampaignMain.campaignMain.isUsingAdvanceRepair()) {
                ...
                if (!this.hasRepairingUnits()) {
                    CampaignMain.campaignMain.toUser("PL|UTT|" + totalTechsToString(), name, false);
                    CampaignMain.campaignMain.toUser("PL|UAT|" + totalTechsToString(), name, false);
    ...
    /**
     * Issue with vacant pilots getting placed in !Mek and !Proto Units This fixes it. Will also be helpful if future
     * bugs cause vacant pilots.
     *
     * @param unit
     */
    ...
    public void updatePartsCache(String part, int amount) {
        ...
        CampaignMain.campaignMain.toUser("PL|UPPC|" + part + "#" + amount, getName(), false);
    ...
    public void checkForPromotion() {
        ...
        if (CampaignMain.campaignMain.getBooleanConfig("Disable_Promote_Subfaction")) //@salient
        {
            return;
        }
        ...
        for (SubFaction subFaction : getMyHouse().getSubFactionList().values()) {
            ...
            if ((currentAccessLevel < Integer.parseInt(subFaction.getConfig("AccessLevel"))) &&
                ...
                CampaignMain.campaignMain.toUser("You are eligible for a promotion to subFaction " +
                                                       subFaction.getConfig("Name") +
                                                       ". <a href=\"MEKWARS/c RequestSubFactionPromotion#" +
                                                       subFaction.getConfig("Name") +
                                                       "\">Click here to request promotion.</a>", getName());
            }
    ...
    public void checkForDemotion() {
        ...
        if (CampaignMain.campaignMain.getBooleanConfig("Disable_Demote_Subfaction")) //@salient
        {
            return;
        }
        ...
        if (CampaignMain.campaignMain.getBooleanConfig("autoPromoteSubFaction")) {
            ...
            CampaignMain.campaignMain.toUser("PL|SSN|" + subFactionName, getName(), false);
            CampaignMain.campaignMain.doSendToAllOnlinePlayers("PI|FT|" + getName() + "|" + getFluffText(), false);
            CampaignMain.campaignMain.toUser("HS|CA|0", getName(), false);// clear
            // old
            // data
            CampaignMain.campaignMain.toUser(getMyHouse().getCompleteStatus(), getName(), false);
            for (SArmy army : getArmies()) {
            ...
            CampaignMain.campaignMain.toUser("AM:You have been demoted to SubFaction " + subFactionName + ".",
                  getName());
            CampaignMain.campaignMain.doSendHouseMail(getMyHouse(),
                  "NOTE",
                  getName() + " has been demoted to subfaction " + subFactionName + " by the Faction Leadership!");
            return;
        }

        if (((elo > getRating()) || (exp > getExperience())) &&
                  !CampaignMain.campaignMain.getBooleanConfig("disableDemotionNotification") &&
                  !CampaignMain.campaignMain.getBooleanConfig("autoPromoteSubFaction")) {
    ...
    /**
     * A player may only have 1 army locked at a time. This will lock that army and unlock any others Passing an armyId
     * of -1 will unlock all armies.
     *
     * @param armyId
     */
    public void lockArmy(int armyId) {
        ...
        for (SArmy army : getArmies()) {
            ...
            if (army.getID() == armyId) {
                army.setLocked(true);
                CampaignMain.campaignMain.toUser("PL|SAL|" + armyId + "#" + true, getName(), false);
            } else if (army.isLocked()) {
                army.setLocked(false);
                CampaignMain.campaignMain.toUser("PL|SAL|" + army.getID() + "#" + false, getName(), false);
            }
        }
    }
    ...
    /**
     * A method to determine if a player is above or below the hangar limits for units, based on type and weight.
     *
    ...
    /**
     * A method to determine if the player will be over the unit limit after purchasing a new unit if the server is
     * configured to use sliding hangar cost increases
     *
     */
