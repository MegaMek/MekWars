 ...
 *
 * @author Helge Richter (McWizard) Jun 10/04 - Dave Poole added an overloaded constructor to allow creation of a new
 *       SUnit with the same UnitID as an existing Mech to facilitate repodding
 */
...
public final class SUnit extends Unit implements Comparable<SUnit> {
    ...
    public SUnit(int replaceId, String p, String Filename) {
        super();
        setUnitFilename(Filename);
        Entity ent = mekwars.server.campaign.SUnit.loadMech(getUnitFilename());
        setEntity(ent);
        init();
    ...
    /**
     * Method which checks a unit for illegal ammo and replaces it with default ammo loads. useful for removing faction
     * banned ammo from salvage. Note that this is primarily designed to strip L2 ammo from L2 units (eg - precision AC)
     * and replace it with normal ammo. L3 ammos may lead to some oddities and should be banned or allowed server wide
     * rather than on a house-by-house basis.
     *
    ...
    public static int getHangarSpaceRequired(int typeid, int weightclass, int baymod, String model,
          boolean unitSupported, SHouse faction) {
        if (unitSupported) {
            return mekwars.server.campaign.SUnit.getHangarSpaceRequired(typeid, weightclass, baymod, model, faction);
        }
        return (int) (mekwars.server.campaign.SUnit.getHangarSpaceRequired(typeid,
              weightclass,
              baymod,
              model,
    ...
    /**
     * Pass-through method that gets the number of bays/techs required for a given unit by drawing its characteristics
     * and feeding them to getHangarSpaceRequired(int,int,int,String).
     */
    public static int getHangarSpaceRequired(mekwars.server.campaign.SUnit u, SHouse faction) {
        return mekwars.server.campaign.SUnit.getHangarSpaceRequired(u.getType(),
              u.getWeightclass(),
    ...
    public static int getHangarSpaceRequired(mekwars.server.campaign.SUnit u, boolean unitSupported, SHouse faction) {
        if (unitSupported) {
            return mekwars.server.campaign.SUnit.getHangarSpaceRequired(u.getType(),
                  u.getWeightclass(),
                  u.getPilot().getBayModifier(),
    ...
    /**
     * Simple static method that access configs and returns a unit's influence on map size. Called by ShortOperation
    ...
    public String toString(boolean toPlayer) {
        ...
        if (toPlayer) {
            java.util.LinkedList<MegaMekPilotOption> mmoptions = getPilot().getMegamekOptions();
            msg.append(mmoptions.size());
            java.util.Iterator<MegaMekPilotOption> i = mmoptions.iterator();
            while (i.hasNext()) {
        ...
        java.util.ArrayList<Mounted> en_Ammo = unitEntity.getAmmo();
    ...
    public String fromString(String s) {
        ...
        try {
            ...
            if (ST.hasMoreElements()) {
                ...
                java.util.ArrayList<Mounted> e = en.getAmmo();
            ...
            if (ST.hasMoreTokens()) {TokenReader.readString(ST);}
    ...
    public String getDescription(boolean showLink) {
        ...
        String dialogBox = "<a href=\"MEKINFO" +
    ...
    public boolean equals(Object o) {
        ...
        mekwars.server.campaign.SUnit m = null;
        try {
            m = (mekwars.server.campaign.SUnit) o;
    ...
    public void setPilot(SPilot p) {
        ...
        if (p.getSkills().has(PilotSkill.WeaponSpecialistSkillID)) {
            java.util.Iterator<PilotSkill> ski = p.getSkills().getSkillIterator();
            while (ski.hasNext()) {
    ...
    /**
     * Sets status to unmaintained. Factors out repetetive code checking maintainance status and decreasing as unit is
     * moved to unmaintained. Called from both Player and SetUnmaintainedCommand. It would possible to bypass this code
     * and set a unit as unmaintained without incurring any maintainance penalty w/
    ...
    public boolean isOmni() {
        ...
        if ((getType() == Unit.VEHICLE) && !isOmni) {
            try {
                java.io.FileInputStream fis = new java.io.FileInputStream("./data/buildtables/omnivehiclelist.txt");
    ...
    public int getPilotSkillBV() {
        ...
        int skillBV = 0;
        java.util.Iterator<PilotSkill> pilotSkills = getPilot().getSkills().getSkillIterator();
     ...
     * @author Spork
     */
    public int compareTo(mekwars.server.campaign.SUnit u) {
