 ...
 * <p>
 * Note, that while you write modifyPilot, you must only add differences between levels. An example:
 * <p>
 * If your Pilot should get a malus to gunnery for each level, you have to add only +1 to gunnery in modifyPilot, since
 * the malus for previous levels are already applied.
 *
...
public abstract class SLevelPilotSkill extends SPilotSkill {
     ...
     */
    public SLevelPilotSkill(int id, String name, mekwars.server.campaign.pilot.skills.SLevelPilotSkill previous,
          String abbreviation) {
        super(id, name, abbreviation);
        this.previous = previous;
        for (mekwars.server.campaign.pilot.skills.SLevelPilotSkill p = previous; p != null; p = p.previous) {level++;}
    }
    ...
    /**
     * Get the chance by asking of the chance of level upgrades. Do not override, but use getUpgradeChance() instead.
     */
    @Override
