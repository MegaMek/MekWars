...
public class ExchangeUnitCommand implements Command {
    ...
    public void process(java.util.StringTokenizer command, String Username) {
        ...
        if (command.hasMoreElements()) {
            ...
            if (a == null) {
                CampaignMain.campaignMain.toUser("AM:You do not have an  Army #" + armyid, Username, true);
                return;
            }

            //Is the Lance in a fight atm?
            if (a.isLocked()) {
                CampaignMain.campaignMain.toUser("AM:This Army is currently in a game.", Username, true);
                return;
            }
            ...
            if (changeMech != null && changeMech.getModelName().startsWith("Error")) {
                CampaignMain.campaignMain.toUser("AM:Error units may not be added to armies.", Username, true);
                return;
            }

            if (changeMech != null && a.isUnitInArmy(changeMech)) {
                CampaignMain.campaignMain.toUser("AM:That unit already exits in Army #" + armyid + ".",
                      Username,
                      true);
                return;
            }
            ...
            if (p.getDutyStatus() == server.campaign.SPlayer.STATUS_ACTIVE) {
                CampaignMain.campaignMain.toUser("AM:You may not change your armies while on active duty.",
                      Username,
                      true);
                return;
            }
            ...
            if (changeMech != null && changeMech.getStatus() == Unit.STATUS_UNMAINTAINED) {
                CampaignMain.campaignMain.toUser("AM:You may not assign unmaintained units to combat formations!",
                      Username,
                      true);
                return;
            }
            ...
            if (changeMech != null) {
                int maxAmount = Integer.valueOf(CampaignMain.campaignMain.getConfig("UnitsInMultipleArmiesAmount"))
                                      .intValue();
                if (p.getAmountOfTimesUnitExistsInArmies(changeMech.getId()) >= maxAmount) {
                    if (maxAmount == 1) {
                        CampaignMain.campaignMain.toUser("AM:A unit may only be in one army at a time.",
                              Username,
                              true);
                    } else {
                        CampaignMain.campaignMain.toUser("AM:A unit may be in a maximum of " +
                                                               maxAmount +
                ...
                if (oldMech != null) {
                    ...
                    a.removeUnit(oldID);
                    CampaignMain.campaignMain.toUser("PL|RAU|" + a.getID() + "#" + oldID + "#" + a.getBV(),
                          Username,
                          false);
                    CampaignMain.campaignMain.toUser("PL|UU|" + oldMech.getId() + "|" + oldMech.toString(true),
                          Username,
                          false);
                ...
                if (position > -1) {
                    ...
                    CampaignMain.campaignMain.toUser("PL|AAU|" +
                                                           a.getID() +
                                                           "#" +
                                                           changeMech.getId() +
                                                           "#" +
                ...
                CampaignMain.campaignMain.toUser("PL|RAU|" + a.getID() + "#" + oldMech.getId() + "#" + a.getBV(),
                      Username,
                      false);
                a.checkLegalRatio(Username);
                CampaignMain.campaignMain.toUser("PL|UU|" + oldMech.getId() + "|" + oldMech.toString(true),
            ...
            CampaignMain.campaignMain.toUser("AM:Army #" + a.getID() + " was changed. New BV: " + a.getBV(),
                  Username,
                  true);
            CampaignMain.campaignMain.getOpsManager().checkOperations(a, true);
