 ...
 * <p>
 * This command allows a player to freely create a unit, which is then dropped into his hangar. It also checks the build
 * table to ensure that a legal unit has been requested. This was originally created for new players only, however it
 * has been expanded to allow the player to create their starting hangar after defecting to a new faction.
 *
 * @Salient 2017.9.01
 */
public class FreeBuildCreateUnitCommand implements Command {
    ...
    private java.util.List<String> houseList = new java.util.ArrayList<String>();
    private server.campaign.SPlayer player;
    private server.campaign.SHouse house;
    private server.campaign.SUnit unit;
    ...
    private server.campaign.SUnit readCommandReturnSUnit(java.util.StringTokenizer command) {
        ...
        server.campaign.SUnit tempUnit = server.campaign.SUnit.create(filename,
              FlavorText,
              house.getBaseGunner(),
              house.getBasePilot(),
              weight,
    ...
    private Boolean playerUnitLimitChecks() {
        ...
        if (!allowDupes) {
            java.util.Vector<server.campaign.SUnit> playersUnits = player.getUnits();
    ...
    private Boolean checkDupeLimits(int unitCount) {
        switch (unit.getType()) {
            ...
            case 6:
                ...
                if (dupeLimitMeks > unitCount) {return true;} else {
                    player.toSelf(
            ...
            case 1:
                ...
                if (dupeLimitVees > unitCount) {return true;} else {
                    player.toSelf(
            ...
            case 2:
                ...
                if (dupeLimitInf > unitCount) {return true;} else {
                    player.toSelf(
            ...
            case 4:
                ...
                if (dupeLimitBA > unitCount) {return true;} else {
                    player.toSelf(
            ...
            case 5:
                ...
                if (dupeLimitAero > unitCount) {return true;} else {
                    player.toSelf(
    ...
    /**
     * Get a collection of all houses in game and covert it to a list Need to set the list of houses before calling
     * checkiflegal method
     */
    ...
    /**
     * add non-faction build tables to list if they aren't already present also add the table that is set by the SO
     */
     ...
     */
    private Boolean CheckIfLegal(String buildTableName, server.campaign.SUnit unitToCheck) throws java.io.IOException {

        Boolean result = false;

        buildTableName += "_" + server.campaign.SUnit.getWeightClassDesc(unitToCheck.getWeightclass());
        ...
        if (unitToCheck.getType() != 0) {
            buildTableName += server.campaign.SUnit.getTypeClassDesc(unitToCheck.getType());
        }
        ...
        java.nio.file.Path path = java.nio.file.Paths.get("data/buildtables/standard/" + buildTableName)
                                        .toAbsolutePath();
        ...
        java.util.List<String> allowedUnits = java.nio.file.Files.lines(path)
