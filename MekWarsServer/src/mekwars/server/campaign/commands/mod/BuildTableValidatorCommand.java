...
/**
 * This command just goes through the build lists and makes sure all of the files on the build list will load with what
 * is in the servers zip files.
 *
...
public class BuildTableValidatorCommand implements server.campaign.commands.Command {
    ...
    private String validate(String folderName, String Username) {
        ...
        java.io.File file = new java.io.File("./data/buildtables/" + folderName);
        ...
        for (int i = 0; i < fileList.length; i++) {
            ...
            try {
                ...
                java.io.FileReader fis = new java.io.FileReader(fileList[i]);
                dis = new java.io.BufferedReader(fis);
                while (dis.ready()) {
                    ...
                    if (!lowerLine.endsWith("mtf")
                              ...
                              && !line.endsWith("hmv")
                              && !line.endsWith("xml")) {
                        toReturn.append("Unknown file extension? " +
                                              line +
                                              "[Table: " +
                                              fileList[i].getPath() +
                                              "]<br>");
                    ...
                    Entity ent = server.campaign.SUnit.loadMech(unitToBuild);
                    if (ent.getModel().equals("OMG-UR-FD")) {
                        toReturn.append("Error loading: " +
                                              unitToBuild.trim() +
                                              " [Table: " +
                                              fileList[i].getPath() +
                                              "]<br>");
