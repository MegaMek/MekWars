    ...
    public void process(java.util.StringTokenizer command, String Username) {
        ...
        while (commands.hasMoreElements()) {
            ...
            if (starter.trim().length() > 0) {
                if (userLevel < commandMethod.getExecutionLevel()) {continue;} else if (commandName.toLowerCase()
                                                                                              .indexOf(starter.toLowerCase()) >
                                                                                              -1) {
                    String syntax = commandMethod.getSyntax().trim().length() < 1 ? " " : commandMethod.getSyntax();
                    commandArray[x] = commandName.substring(0, 1) +
        ...
        java.util.Arrays.sort(commandArray);
        ...
        for (x = 0; x < commandArray.length; x++) {
            java.util.StringTokenizer commandList = new java.util.StringTokenizer(commandArray[x], "~");
            result.append("<tr><td>" +
