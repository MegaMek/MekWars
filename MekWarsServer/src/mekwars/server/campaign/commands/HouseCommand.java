...
public class HouseCommand implements Command {
    ...
    public void process(java.util.StringTokenizer command, String Username) {
        ...
        if (!command.hasMoreElements()) {
            CampaignMain.campaignMain.toUser("AM:Improper format. Try: /c faction#name", Username, false);
            return;
        }
        ...
        server.campaign.SHouse h = (server.campaign.SHouse) CampaignMain.campaignMain.getData()
                                                                  .getHouseByName(Name);
        if (h == null || h.getId() < 0) {
            CampaignMain.campaignMain.toUser("AM:Could not find faction. Command fails.", Username, false);
            return;
        }
        ...
        if (CampaignMain.campaignMain.getPlayer(Username).getMyHouse().equals(h) ||
                  CampaignMain.campaignMain.getServer().isModerator(Username)) {
            s += " Current Research: " +
                       h.getTechResearchPoints() +
                       " of " +
                       CampaignMain.campaignMain.getConfig("TechPointsNeedToLevel");
        }
        ...
        java.util.Iterator<server.campaign.operations.ShortOperation> games = CampaignMain.campaignMain.getOpsManager()
                                                                                    .getRunningOps()
                                                                                    .values()
                                                                                    .iterator();
        while (games.hasNext()) {
