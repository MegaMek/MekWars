...
public class RequestCommand implements Command {
    ...
    public void process(java.util.StringTokenizer command, String Username) {
        ...
        server.campaign.SUnitFactory factory = null;
        ...
        if (command.hasMoreElements()) {
             ...
             */
            for (server.campaign.SUnitFactory currFac : namedFactories) {
        ...
        if (hasEnoughMoney && hasEnoughInfluence && factionHasEnoughPP) {
            ...
            // requested
            int spaceTaken = server.campaign.SUnit.getHangarSpaceRequired(type_id,
                  weightclass,
                  0,
                  "null",
                  p.getMyHouse());
            ...
            java.util.Vector<server.campaign.SUnit> mechs = factory.getMechProduced(type_id, pilot);
            StringBuffer results = new StringBuffer();
            if (playerHouse.getBooleanConfig("UseCalculatedCosts")) {
                mechCbills = 0;
            }
            for (server.campaign.SUnit mech : mechs) {
                if (playerHouse.getBooleanConfig("UseCalculatedCosts")) {
