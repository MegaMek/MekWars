...
public class DirectSellUnitCommand implements Command {
    ...
    public void process(java.util.StringTokenizer command, String Username) {
        ...
        if (p.mayAcquireWelfareUnits()) {
            CampaignMain.campaignMain.toUser("AM:You may not sell any of you units while you are on welfare.",
                  Username,
                  true);
            return;
        }
        ...
        server.campaign.SPlayer pBuyer = CampaignMain.campaignMain.getPlayer(buyer);
        server.campaign.SPlayer pSeller = CampaignMain.campaignMain.getPlayer(seller);
        boolean usesTechs = CampaignMain.campaignMain.getBooleanConfig("UseTechnicians");

        if (!CampaignMain.campaignMain.getBooleanConfig("UseDirectSell")) {
            CampaignMain.campaignMain.toUser("AM:Direct Sell is not allowed on this server!", Username, true);
            return;
        }

        if (pSeller == null) {
            CampaignMain.campaignMain.toUser("AM:Selling player (" + seller + ") could not be found.",
                  Username,
                  true);
            return;
        }
        ...
        if (pBuyer == null) {
            CampaignMain.campaignMain.toUser("AM:Buying player (" + buyer + ") could not be found.",
                  Username,
                  true);
            return;
        }
        ...
        if (pSeller.getMyHouse().isNewbieHouse()) {
            CampaignMain.campaignMain.toUser("AM:Players in SOL may not direct sell units.", Username, true);
            return;
        }
        ...
        if (m.getStatus() == Unit.STATUS_FORSALE) {
            CampaignMain.campaignMain.toUser("AM:You may not directly sell units already on the open market.",
                  Username,
                  true);
            return;
        }

        if (m.isChristmasUnit() && !CampaignMain.campaignMain.getBooleanConfig("Christmas_AllowDirectSell")) {
            CampaignMain.campaignMain.toUser("AM:You may not sell Christmas units.", Username);
            return;
        }
        ...
        if (pBuyer.getFreeBays() < server.campaign.SUnit.getHangarSpaceRequired(m, pBuyer.getMyHouse()) && !usesTechs) {
            ...
            // unmaintained
            CampaignMain.campaignMain.toUser(pBuyer.getName() + " has no room for that unit.", seller, true);
            return;
            // Target is not logged in?
        } else if (!pBuyer.getMyHouse().isLoggedIntoFaction(buyer)) {
            CampaignMain.campaignMain.toUser(pBuyer.getName() +
                                                   " is not logged in. You may only transfer to pSellers who are online.",
                  Username,
                  true);
            return;
            // Same IP address?
        } else if (CampaignMain.campaignMain.getBooleanConfig("IPCheck")) {
            if (CampaignMain.campaignMain.getServer().getIP(pSeller.getName()).toString().equals(
                  CampaignMain.campaignMain.getServer().getIP(pBuyer.getName()).toString())) {
                CampaignMain.campaignMain.toUser(pBuyer.getName() +
            ...
            CampaignMain.campaignMain.toUser("AM:You may not sell unmaintained mechs.", Username, true);
            return;
        } else if (pSeller.getAmountOfTimesUnitExistsInArmies(m.getId()) > 0 &&
                         pSeller.getDutyStatus() == server.campaign.SPlayer.STATUS_ACTIVE) {
            CampaignMain.campaignMain.toUser("AM:You may not sell units which are in active armies.",
                  Username,
                  true);
            return;
        ...
        for (server.campaign.SArmy currA : pSeller.getArmies()) {
            if (currA.isLocked() && currA.getUnit(m.getId()) != null) {
                CampaignMain.campaignMain.toUser("AM:You may not sell units which are in fighting armies.",
                      pSeller.getName(),
                      true);
                return;
            }
        ...
        String basePayment = "SellDirect" +
                                   ...
                                   "Price";
        int transferPayment = CampaignMain.campaignMain.getIntegerConfig(basePayment);
        ...
        if (!confirmedSend) {
            if (Username.equals(buyer)) {
                CampaignMain.campaignMain.toUser("AM:Illegal transaction! The staff was alerted!", buyer, true);
            ...
            CampaignMain.campaignMain.toUser(seller +
        ...
        if (pBuyer.getMoney() < costToRecipient + 1) {
            CampaignMain.campaignMain.toUser("AM:You tried to sell " +
                                                   StringUtils.aOrAn(m.getModelName(), true) +
                                                   " to " +
                                                   pBuyer.getName() +
                                                   ", but they " +
        ...
        String result = "AM:The " +
                              ...
                              pBuyer.getName() +
                              " for " +
                              CampaignMain.campaignMain.moneyOrFluMessage(true, true, costToRecipient);
        ...
        CampaignMain.campaignMain.toUser(result, seller, true);
        CampaignMain.campaignMain.toUser(pSeller.getName() +
                                               " has sold you " +
                                               StringUtils.aOrAn(m.getModelName(), true) +
                                               " for " +
                                               CampaignMain.campaignMain.moneyOrFluMessage(true,
