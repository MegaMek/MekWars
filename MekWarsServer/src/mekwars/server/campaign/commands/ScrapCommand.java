...
public class ScrapCommand implements Command {
    ...
    public void process(java.util.StringTokenizer command, String Username) {
        ...
        if (scrapsAllowed <= 0) {
            CampaignMain.campaignMain.toUser("AM:Scrapping is not allowed on this server.", Username, true);
            return;
        }

        if (house.isNewbieHouse()) {
            CampaignMain.campaignMain.toUser("AM:SOL players may not Sell, Scrap or Donate their units!",
                  Username,
                  true);
            return;
        }
        ...
        if (p.mayAcquireWelfareUnits()) {
            CampaignMain.campaignMain.toUser("AM:You may not scrap any of your units while you are on welfare.",
                  Username,
                  true);
            return;
        }
        ...
        try {
            ...
            CampaignMain.campaignMain.toUser("AM:Formatting error. Try: /c scrap#ID Number", Username, true);
            return;
        }
        ...
        if (m.getStatus() == Unit.STATUS_FORSALE) {
            CampaignMain.campaignMain.toUser("AM:Units that are for sale on the Market may not be scrapped.",
                  Username,
                  true);
            return;
        }

        if (m.isChristmasUnit() && !CampaignMain.campaignMain.getBooleanConfig("Christmas_AllowScrap")) {
            CampaignMain.campaignMain.toUser("AM:Scrapping a Christmas gift?  Bad form.", Username);
            return;
        }

        if (p.getAmountOfTimesUnitExistsInArmies(mechid) > 0 &&
                  p.getDutyStatus() == server.campaign.SPlayer.STATUS_ACTIVE) {
            CampaignMain.campaignMain.toUser("AM:You may not scrap units which are in active armies.",
                  Username,
                  true);
            return;
        ...
        if (p.isUnitInLockedArmy(m.getId())) {
            CampaignMain.campaignMain.toUser("AM:You may not scrap units which are in fighting armies.",
                  Username,
                  true);
            return;
        }
        ...
        if (p.getScrapsThisTick() >= scrapsAllowed && m.getScrappableFor() <= 0) {
            CampaignMain.campaignMain.toUser("AM:You may only scrap " + scrapsAllowed + " unit(s) per tick.",
                  Username,
                  true);
            return;
        }
        ...
        float costMulti = 0;
        if (!CampaignMain.campaignMain.isUsingAdvanceRepair() || m.getType() != Unit.MEK) {
        ...
        if (p.getMoney() < moneyToScrap || p.getInfluence() < infToScrap && m.getScrappableFor() < 0) {
            CampaignMain.campaignMain.toUser("AM:You cannot afford to scrap this unit. You need " +
        ...
        if (m.getScrappableFor() >= 0) {
            ...
            CampaignMain.campaignMain.toUser("AM:You scrapped the " +
                                                   m.getModelName() +
                                                   " (" +
                                                   CampaignMain.campaignMain.moneyOrFluMessage(true,
                                                         true,
            ...
            if (!strConfirm.equals("CONFIRM")) {
                String result = "AM:Quartermaster command will charge you " +
                                      CampaignMain.campaignMain.moneyOrFluMessage(true, false, moneyToScrap) +
                                      " to scrap #" +
                                      m.getId() +
                                      " " +
                                      m.getModelName() +
        ...
        server.campaign.operations.OpsScrapThread scrapT = CampaignMain.campaignMain.getOpsManager()
                                                                 .getScrapThreads()
        ...
        if (Boolean.parseBoolean(house.getConfig("AllowPersonalPilotQueues"))
            ...
            CampaignMain.campaignMain.toUser(pilot.getName() + " was moved to your barracks.", Username, true);
        ...
        CampaignMain.campaignMain.addMechStat(m.getUnitFilename(), m.getWeightclass(), 0, 0, 1);
