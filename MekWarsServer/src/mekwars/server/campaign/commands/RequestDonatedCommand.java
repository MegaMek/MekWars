...
public class RequestDonatedCommand implements Command {
    ...
    public void process(java.util.StringTokenizer command, String Username) {
        ...
        if (p.mayAcquireWelfareUnits()) {
            server.campaign.SUnit unit = buildWelfareMek(house.getName());
        ...
        // to be had.
        server.campaign.SUnit u = house.getEntity(weightclass, type_id);
        ...
        if (!Boolean.parseBoolean(p.getSubFaction()
                                        .getConfig("CanBuyUsed" +
                                                         Unit.getWeightClassDesc(weightclass) +
                                                         Unit.getTypeClassDesc(type_id)))) {
        ...
        int unitCbills = Math.round(Float.parseFloat(house.getConfig("UsedPurchaseCostMulti")) *
                                          house.getPriceForUnit(u.getWeightclass(), u.getType()));
        int unitInfluence = Math.round(Float.parseFloat(house.getConfig("UsedPurchaseCostMulti")) *
        ...
        if (CampaignMain.campaignMain.isUsingAdvanceRepair()) {
            if (!UnitUtils.canStartUp(u.getEntity())) {
                ...
                unitCbills = Math.round(unitCbills *
                                              Float.parseFloat(house.getConfig("CostModifierToBuyCritDamagedUnit")));
            } else if (UnitUtils.hasArmorDamage(u.getEntity())) {
                unitCbills = Math.round(unitCbills *
                                              Float.parseFloat(house.getConfig("CostModifierToBuyArmorDamagedUnit")));
            }
        }
        ...
        int spaceTaken = server.campaign.SUnit.getHangarSpaceRequired(type_id, weightclass, 0, "null", p.getMyHouse());
     ...
     * to ensure that ONLY requests generate welfare units (had previously been a public call in CampaignMain).
     */
    private server.campaign.SUnit buildWelfareMek(String producer) {
        ...
        String unitFileName = server.campaign.BuildTable.getUnitFilename(Filename);
        server.campaign.SUnit cm = new server.campaign.SUnit(producer, unitFileName, Unit.LIGHT);
