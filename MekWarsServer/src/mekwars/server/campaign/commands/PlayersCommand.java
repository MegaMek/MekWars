    ...
    public void process(java.util.StringTokenizer command, String Username) {
        ...
        for (House vh : server.campaign.CampaignMain.cm.getData().getAllHouses()) {
            ...
            if (h.getFightingPlayers().size() > 0) {
                ...
                java.util.TreeMap<String, server.campaign.SPlayer> sortedPlayers = new java.util.TreeMap<String, server.campaign.SPlayer>();
                sortedPlayers.putAll(h.getFightingPlayers());

                toSend += "<b>" + h.getColoredNameAsLink() + ":</b> ";
                for (server.campaign.SPlayer currP : sortedPlayers.values()) {toSend += currP.getName() + ", ";}
        ...
        if (!(Boolean.parseBoolean(server.campaign.CampaignMain.cm.getConfig("HideActiveStatus")))) {
            ...
            for (House vh : server.campaign.CampaignMain.cm.getData().getAllHouses()) {
                ...
                if (h.getReservePlayers().size() > 0) {
                    ...
                    toSend += ("<b>" + h.getColoredNameAsLink() + ":</b> ");
                    for (server.campaign.SPlayer currP : sortedPlayers.values()) {
         ...
         */
            ...
            for (House vh : server.campaign.CampaignMain.cm.getData().getAllHouses()) {
                ...
                if (combinedTable.size() > 0) {
                    ...
                    //add all players
                    toSend += ("<b>" + h.getColoredNameAsLink() + ":</b> ");
                    for (server.campaign.SPlayer currP : combinedTable.values()) {

                        if (currP.isInvisible()) {continue;}

                        if ((playersFaction || isAdmin) &&
                                  currP.getDutyStatus() == server.campaign.SPlayer.STATUS_ACTIVE) {
