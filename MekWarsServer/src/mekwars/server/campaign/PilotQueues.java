...
public class PilotQueues {
    ...
    private java.util.Vector<java.util.LinkedList<SPilot>> queues = new java.util.Vector<java.util.LinkedList<SPilot>>(
          Unit.MAXBUILD,
          1);
    private java.util.Vector<Integer> baseGunnery = new java.util.Vector<Integer>(Unit.MAXBUILD, 1);
    private java.util.Vector<Integer> basePiloting = new java.util.Vector<Integer>(Unit.MAXBUILD, 1);
    private java.util.Vector<String> basePilotSkills = new java.util.Vector<String>(Unit.MAXBUILD, 1);
    private String factionString = "";//string for faction specific name list
    private int factionID;
    private java.io.BufferedReader dis;

    public PilotQueues(java.util.Vector<Integer> baseGunnery, java.util.Vector<Integer> basePiloting,
          java.util.Vector<String> basePilotSkill) {
        for (int i = Unit.MEK; i < Unit.MAXBUILD; i++) {
            java.util.LinkedList<SPilot> v = new java.util.LinkedList<SPilot>();
            queues.add(i, v);
        }
     ...
     *                        <p>
     *                        This particular add mechanism should only be called from *within* the PilotQueues class or
     *                        when captureing a pilot. It is used to add newly generated pilots and captured enemy pilot
     *                        to the queue, with their level ups (server gen'ed 3/5 and 4/4 skills, and other oddball
     ...
     *             <p>
     *             Check config to see if pilots should always retain their skills. If not, check for an elite pilot and
     *             downshift his abilities. Green units should not be improved; however, a 4/5 pilot will be set in
     *             place of a pilot with 5 gunnery and improved piloting (no 5/2, 5/3). Also check config to see if
     *             pilot XP should have a percentage chance to be cleared w/ skills.
     *
     * @urgru 6/07/04
     */
     ...
     * @return a pilot name
     *       <p>
     *       NOTE: this is modelled on getRandomPilotName() from SUnit, but DOES NOT replace it. SUnit's naming defaults
     *       to the common list and is used for newbie faction units and for units generated with CreateMechCommand.
     */
    public String getRandomPilotName() {
        ...
        try {
            java.io.File configFile = new java.io.File("./data/pilotnames/" + factionString + "Pilotnames.txt");
