...
public class UserActivityInfluenceJob implements Job, MWRepeatingJob, JobIdentifiableByUser {
    private static final MMLogger LOGGER = MMLogger.create(UserActivityInfluenceJob.class);
    ...
    public static void submit(String userName, Double weightedArmyValue, String factionName, int frequency) {
        JobDetail job = newJob(mekwars.server.campaign.util.scheduler.UserActivityInfluenceJob.class)
                              .withIdentity(userName + "_flu", "ActivityGroup")
                              .build();

        Trigger trigger = newTrigger()
                                .withIdentity(userName + "_fluTrigger", "ActivityGroup")
                                .startAt(new java.util.Date(java.util.Calendar.getInstance().getTimeInMillis() +
                                                                  frequency * 1000))
     ...
     */
    private boolean userGetsInfluence(server.campaign.SPlayer p) {
        ...
        boolean activeLongEnough = (System.currentTimeMillis() - p.getActiveSince()) >
    ...
    /**
     * A method to calculate how much influence the player will receive, based on the army weight, and the settings of
     * the server
     *
     * @param p - the player
     *
     ...
     */
    private int calculateInfluence(server.campaign.SPlayer p) {
     ...
     * @return the message
     */
    private String getInfluenceMessage(server.campaign.SPlayer p, int flu) {
        String fileName = "";
        server.campaign.SHouse faction = p.getHouseFightingFor();
        ...
        java.io.File messageFile = new java.io.File(fileName);
        if (!messageFile.exists()) {
            fileName = "./data/influencemessages/CommonInfluenceMessages.txt";
            messageFile = new java.io.File(fileName);
            if (!messageFile.exists()) {
        ...
        try {
            fis = new java.io.FileInputStream(messageFile);
        ...
        java.util.Vector<server.campaign.SUnit> units = p.getUnits();
        int unitId = rand.nextInt(units.size());// random
        server.campaign.SUnit unitForMessages = units.elementAt(unitId);
    ...
    public void rescheduleByContext(int seconds, JobExecutionContext context) {
        ...
        mekwars.server.campaign.util.scheduler.UserActivityInfluenceJob.stop(playerName);
        mekwars.server.campaign.util.scheduler.UserActivityInfluenceJob.submit(playerName,
              armyWeight,
              factionName,
              seconds);
    }
    ...
    /**
     * A method to reschedule a particular user's influence job at a given frequency This is called if the admin changes
     * the influence frequency, so users are not forced inactive.
     *
