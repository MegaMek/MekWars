...
public class UserActivityComponentsJob implements Job, MWRepeatingJob, JobIdentifiableByUser {
    ...
    public static void submit(String userName, Double weightedArmyValue, String factionName, int frequency) {
        JobDetail job = newJob(mekwars.server.campaign.util.scheduler.UserActivityComponentsJob.class)
        ...
        job.getJobDataMap()
              .put(mekwars.server.campaign.util.scheduler.UserActivityComponentsJob.ARMY_WEIGHT,
                    Double.toString(weightedArmyValue));
        job.getJobDataMap()
              .put(mekwars.server.campaign.util.scheduler.UserActivityComponentsJob.FACTION_NAME, factionName);
        job.getJobDataMap().put(mekwars.server.campaign.util.scheduler.UserActivityComponentsJob.PLAYER_NAME, userName);
     ...
     */
    private boolean playerCountsForProduction(server.campaign.SPlayer p) {
        if (p.getWeightedArmyNumber() <= 0) {
            return false;
        }

        if (System.currentTimeMillis() <
                  p.getActiveSince() + Long.parseLong(p.getMyHouse().getConfig("InfluenceTimeMin"))) {
            return false;
        }
     ...
     */
    private double howMuch(server.campaign.SPlayer p) {
        if (p.getDutyStatus() == server.campaign.SPlayer.STATUS_ACTIVE) {
    ...
    public void rescheduleByContext(int seconds, JobExecutionContext context) {
        ...
        mekwars.server.campaign.util.scheduler.UserActivityComponentsJob.stop(playerName);
        mekwars.server.campaign.util.scheduler.UserActivityComponentsJob.submit(playerName,
              armyWeight,
              factionName,
              seconds);
    ...
    /**
     * A method to reschedule a player's component job at a given frequency This is called when the admin changes the
     * frequency of the jobs, so the users do not have to be deactivated.
     *
