...
public class OperationWriter {
    ...
    //METHODS
    public void writeOpList(java.util.TreeMap<String, Operation> ops) {
        ...
        try {
            java.io.PrintStream ps = new java.io.PrintStream(new java.io.FileOutputStream(path));
            ...
            for (Operation currO : ops.values()) {
                ...
                String minOwnIBD = currO.getValue("MinPlanetOwnershipIgnoredByDefender") +
                ...
                if (Boolean.parseBoolean(currO.getValue("OnlyAgainstFactoryWorlds"))) {
                    facInfo = "only*";
                } else if (Boolean.parseBoolean(currO.getValue("OnlyAgainstNonFactoryWorlds"))) {facInfo = "none*";}

                String homeworldInfo = "any*";
                if (Boolean.parseBoolean(currO.getValue("OnlyAgainstHomeWorlds"))) {
                    homeworldInfo = "only*";
                } else if (Boolean.parseBoolean(currO.getValue("OnlyAgainstNonHomeWorlds"))) {homeworldInfo = "none*";}
                ...
                if (!currO.getValue("LegalDefendFactions").trim().equals("")) {
                    ...
                    java.util.StringTokenizer illegalTokenizer = new java.util.StringTokenizer(currO.getValue(
                          "IllegalDefendFactions").trim(), "$");
                    java.util.TreeSet<String> illegals = new java.util.TreeSet<String>();
                ...
                if (allowPlanetFlags.length() < 1) {allowPlanetFlags.append("^ ^*");} else {
                    allowPlanetFlags.append("*");
                }

                StringBuffer disallowPlanetFlags = new StringBuffer(currO.getValue("DisallowPlanetFlags"));
                if (disallowPlanetFlags.length() < 1) {disallowPlanetFlags.append("^ ^*");} else {
