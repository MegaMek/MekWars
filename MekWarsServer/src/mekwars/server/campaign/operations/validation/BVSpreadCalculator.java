...
public class BVSpreadCalculator extends AbstractSpreadCalculator implements I_SpreadCalculator {
    ...
    public static int calcMax(server.campaign.SArmy a, boolean countSupport, boolean countInfantry,
        ...
        for (Unit u : a.getUnits()) {
            if (countUnit(u, countSupport, countInfantry, countVehicles, countAero, countProtos)) {
                if (ignorePilot) {
                    max = Math.max(max, ((server.campaign.SUnit) u).getBaseBV());
                } else {
                    max = Math.max(max, ((server.campaign.SUnit) u).getBVForMatch());
                }
            }
        }
        return max;
    ...
    public static int calcMin(server.campaign.SArmy a, boolean countSupport, boolean countInfantry,
        ...
        for (Unit u : a.getUnits()) {
            if (countUnit(u, countSupport, countInfantry, countVehicles, countAero, countProtos)) {
                if (ignorePilot) {
                    min = Math.min(min, ((server.campaign.SUnit) u).getBaseBV());
                } else {
                    min = Math.min(min, ((server.campaign.SUnit) u).getBVForMatch());
                }
            }
        }
        return min;
