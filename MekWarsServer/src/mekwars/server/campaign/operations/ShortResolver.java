...
public class ShortResolver {
    private static final MMLogger LOGGER = MMLogger.create(ShortResolver.class);
     ...
     */
    java.util.TreeMap<String, server.campaign.SPlayer> allPlayers;

    java.util.TreeMap<String, server.campaign.SArmy> allArmies;

    java.util.TreeMap<String, String> metaStrings;// You Win! messages/meta outcomes.

    java.util.TreeMap<String, String> unitStrings;// salvage, level up and pilot recovery
    // information.

    java.util.TreeMap<String, String> payStrings;// personal payment info (CBills, etc)

    java.util.TreeMap<String, String> longStrings;// long status if part of a long

    java.util.TreeMap<String, Integer> unitCosts;// salvage costs, etc.

    java.util.TreeMap<String, Integer> lossCompensation;// reimbursement for destroyed
    // or enemy-salvaged units.

    java.util.TreeMap<Integer, OperationEntity> salvagableUnits;

    java.util.TreeMap<Integer, OperationEntity> destroyedUnits;

    java.util.TreeMap<Integer, OperationEntity> livingUnits;

    java.util.TreeMap<Integer, SPilot> pilots;

    java.util.TreeMap<String, OpsScrapThread> scrapThreads;// threads are put into
    // OpsMan @ end.
    ...
    /**
     * Method which takes in an autoreport string, reads operation configurations, and sets post-game outcomes as
     * needed. This method does initial processing of the report string, then farms work out to several private methods
     * for salvage, payment, etc. Using the discrete private methods makes it a bit simpler to track all of the
     * paramaters and variables in play ... also allows for automagical disconnection resolution.
     */
    public void resolveShortAttack(Operation o, ShortOperation so, String report) {
        ...
        // master tokenizer
        java.util.StringTokenizer reportTokenizer = new java.util.StringTokenizer(report, "#");
        ...
        java.util.StringTokenizer winnerTokenizer = new java.util.StringTokenizer(reportTokenizer.nextToken(), "*");
        ...
        loserBV = 0;
        attackerBV = 0;
        for (server.campaign.SArmy currArmy : allArmies.values()) {
        ...
        if (so.getWinners().size() == 0) {
            ...
            LOGGER.error("Error while trying to Resolve game: " +
                                  so.getShortID() +
                                  " Result sent by Game: " +
                                  report);
            return;
    ...
    public void init(ShortOperation so) {
         ...
         */
        allPlayers = new java.util.TreeMap<String, server.campaign.SPlayer>();
        allArmies = new java.util.TreeMap<String, server.campaign.SArmy>();
    ...
    public void handleOperationImpact(Operation o, ShortOperation so) {
        ...
        java.util.StringTokenizer wST = new java.util.StringTokenizer(wFlags, "$");
        java.util.StringTokenizer lST = new java.util.StringTokenizer(lFlags, "$");

        while (wST.hasMoreElements()) {
            java.util.StringTokenizer element = new java.util.StringTokenizer(wST.nextToken());
        ...
        while (lST.hasMoreElements()) {
            java.util.StringTokenizer element = new java.util.StringTokenizer(lST.nextToken());
            String flag = element.nextToken();
            String value = element.nextToken();
            boolean shouldApply = true; // if we're not using the new flags, we need to apply this regardless
            if (element.hasMoreTokens()) {
         ...
         */
        for (server.campaign.SPlayer currPlayer : allPlayers.values()) {
    ...
    /**
     * Method which resolves a short operation in the absence of an autoreport - namely, when one player disconnected
     * from a game for an inordinately long period of time. Instead of processing a complete autoreport, this resolution
     * pathway rebuilds the state of the game at the time of the disconnection using a TreeMap which contains the status
     * of all units removed from the game up to that point (destroyed, salvage, etc).
     */
    public void resolveShortAttack(Operation o, ShortOperation so, String winnerName, String loserName) {
        ...
        server.campaign.SArmy winnerA = null;
        server.campaign.SArmy loserA = null;
        if (winner != null) {
        ...
        if ((winnerA == null) || (winnerA.getPlayerName().trim().length() == 0)) {
            LOGGER.error("Autoreporting error [" +
                                  so.getShortID() +
                                  "]:" +
                                  " Winner(" +
                                  winnerName +
        ...
        LOGGER.debug("Autoreporting debug [" +
                                so.getShortID() +
                                "]:" +
                                "break " +
                                loserName +
    ...
    /**
     * Method which determines the payment which should be given to each player, adjusts the payment for salvage costs,
     * and triggers the pay of techs.
     */
    private void assemblePaymentStrings(Operation o, ShortOperation so, server.campaign.SPlayer disconnector) {

        // create the tree
        payStrings = new java.util.TreeMap<String, String>();
        ...
        while (it.hasNext()) {
            ...
            java.util.TreeMap<String, Integer> payout = payoutModifier.calculate(currName,
                  so,
                  earnedMoney,
                  earnedRP,
                  earnedXP,
            ...
            if (earnedXP > 0) {
                ...
                so.checkMercContracts(currP, server.campaign.mercenaries.ContractInfo.CONTRACT_EXP, earnedXP);
                hasOtherGain = true;
            }
        ...
        if ((allPlayers.size() == 2) && opEffectsElo && !drawGame) {
            ...
            server.campaign.SPlayer wp = so.getWinners().get(so.getWinners().firstKey());
            server.campaign.SPlayer lp = so.getLosers().get(so.getLosers().firstKey());
    ...
    /**
     * Method which determines player salvage and generates unique strings. Salvage costs are also determined and saved
     * here; however, they are saved for use in the mathod which generates payStrings rather than shown directly to the
     * players. Determination as to which units are salvageable, which pilots survived the game, etc. should already
     * have been made by one of two methods: - possibleSalvageFromReport(), or - possibleSalvageFromDisconnection
     */
    private void assembleSalvageStrings(Operation o, ShortOperation so) {

        // set up the units cost tree and string tree
        if (null == unitStrings) {
            unitStrings = new java.util.TreeMap<String, String>();
        }
        unitCosts = new java.util.TreeMap<String, Integer>();
        lossCompensation = new java.util.TreeMap<String, Integer>();
        scrapThreads = new java.util.TreeMap<String, OpsScrapThread>();
        ...
        for (OperationEntity currEntity : livingUnits.values()) {
            ...
            if (unitStrings.containsKey(ownerName)) {
                ...
                unitStrings.put(ownerName,
                      s + "The " + currU.getModelName() + " survived the battle" + append + ".<br>");
            } else {
        ...
        for (OperationEntity currEntity : salvagableUnits.values()) {
            ...
            if (winnerAlwaysSalvagesOwn && so.getWinners().containsKey(oldOwnerName)) {
                ...
                for (String currName : allPlayers.keySet()) {
                    ...
                    server.campaign.SPlayer captor = (server.campaign.SPlayer) pilotinformation[4];
    ...
    /**
     * Method which generates general info strings (who attacked whom, which faction won, etc) for the players and a
     * finished status description for public viewing. After these general strings are put together, the need to check
     * Operation values and apply meta effects (% transfers, unit thefts, etc) is checked. If the given ShortOperation
     * is capable of impacting factions (isn't a mult-faction melee, isn't tied to a long operation), the necessary
     * values are loaded from the passed Operation and checked/applied. Some have commented that this is too procedural
     * and not very OO; however, it works. Anyone who cares to beautify the process is more than welcome to do so.
     *
     * @urgru 8.26.05
     */
    private void assembleDescriptionStrings(Operation o, ShortOperation so) {
        ...
        try {// main try for the whole function
            // setup metas
            metaStrings = new java.util.TreeMap<String, String>();

            /*
             * Before we make strings, we need to determine the outcomes.
             * Determine whether the attacking or defending team was the winner.
             */
            server.campaign.SPlayer aWinner = so.getWinners().get(so.getWinners().firstKey());
            server.campaign.SPlayer aLoser = so.getLosers().get(so.getLosers().firstKey());
            // Draw Game
            if (drawGame) {
                attackersWon = false;
                defendersWon = false;
            ...
            if (!drawGame) {
                for (server.campaign.SPlayer wp : so.getWinners().values()) {
                    for (server.campaign.SPlayer compareP : so.getWinners().values()) {
            ...
            for (server.campaign.SPlayer lp : so.getLosers().values()) {
                for (server.campaign.SPlayer compareP : so.getLosers().values()) {
                        ...
                        for (server.campaign.SPlayer wp : so.getWinners().values()) {
                            ...
                            if (numberOfWinners == 1) {
                                metaStrings.put(wp.getName().toLowerCase(),
                                      "You have defeated " + genericLoserString + " (" + so.getName() + ").<br>");
                            } else {
                                int i = 0;
                                int numToList = numberOfWinners - 1;
                                String toSet = "You worked with ";
                                for (server.campaign.SPlayer loopP : so.getWinners().values()) {
                                    if (!wp.equals(loopP)) {
                                        toSet += loopP.getName();
                                        i++;
                                        if (i + 1 == numToList) {
                    ...
                    for (server.campaign.SPlayer lp : so.getLosers().values()) {
                        if (drawGame) {
                            ...
                            metaStrings.put(lp.getName().toLowerCase(),
                                  "You were defeated by " + genericWinnerString + " (" + so.getName() + ").<br>");
                        }
                    }
            ...
            try {
                ...
                // save the planet
                server.campaign.SPlanet target = so.getTargetWorld();
                ...
                if (attackersWon) {
                    try {
                        ...
                        totalConquest = so.getTargetWorld()
                                              .doGainInfluence(aWinner.getHouseFightingFor(),
                        ...
                        if (totalConquest > 0) {
                            ...
                            loserMetaString += " lost " +
                                                     ...
                                                     " of " +
                                                     target.getNameAsColoredLink();
                            so.checkMercContracts(aWinner,
                                  server.campaign.mercenaries.ContractInfo.CONTRACT_LAND,
                                  totalConquest);
                        }
                        ...
                        if ((buildingsLeft > -1) && (buildingsLeft <= so.getTotalBuildings())) {
                            if (attackerisWinner(so)) {
                                buildingsDestroyed = Math.max(buildingsDestroyed,
                        ...
                        if (totalDelay > 0) {
                            for (UnitFactory UF : target.getUnitFactories()) {
                                server.campaign.SUnitFactory currFacility = (server.campaign.SUnitFactory) UF;
                            ...
                            hasLoss = true;
                            so.checkMercContracts(aWinner,
                                  server.campaign.mercenaries.ContractInfo.CONTRACT_DELAY,
                                  totalDelay);
                        }
                        ...
                        if (unitsToCapture > 0) {
                            ...
                            capturedUnits = so.captureUnits(o,
                                  unitsToCapture,
                                  target,
                                  aLoser.getHouseFightingFor(),
                                  false);
                            ...
                            if (numCaptured >= 1) {
                                for (server.campaign.SUnit unit : capturedUnits) {
                                ...
                                // get the units
                                String unitString = "";
                                for (server.campaign.SUnit currU : capturedUnits) {
                            ...
                            so.checkMercContracts(aWinner,
                                  server.campaign.mercenaries.ContractInfo.CONTRACT_UNITS,
                                  numCaptured);
                        ...
                        if (unitsToCapture > 0) {
                            capturedUnits = new java.util.ArrayList<server.campaign.SUnit>();
                            ...
                            if (numCaptured >= 1) {
                                ...
                                if (numUnitsToAward > 0) {
                                    ...
                                    int bvMaxAwardPercent = o.getIntValue(
                                    ...
                                    maxTotalBVToAward = (int) (((double) totalAttackerBV * (double) bvMaxAwardPercent) /
                                                                     100D);
                                ...
                                for (server.campaign.SUnit unit : capturedUnits) {
                                    ...
                                    boolean sendToPlayer = ((numUnitsToAward > numUnitsAwarded) &&
                                ...
                                if (sendToPlayerString.length() > 0) {
                                    ...
                                    unitStrings.put(aWinner.getName().toLowerCase(),
                                ...
                                while (!noPP && (ppCaptured < ppToCapture)) {
                                    ...
                                    if (ppAvailable <= 0) {
                                        ...
                                        if (toTake > 0) {
                                            ...
                                            loserHSUpdates.append(aLoser.getHouseFightingFor()
                                                                        .addPP(currWeight, type, -toTake, false));
                                            winnerHSUpdates.append(aWinner.getHouseFightingFor()
                                                                         .addPP(currWeight, type, toTake, false));
                                            ppCaptured += toTake;
                        ...
                        so.checkMercContracts(aWinner,
                              server.campaign.mercenaries.ContractInfo.CONTRACT_COMPONENTS,
                              ppCaptured);
                        ...
                        java.util.ArrayList<server.campaign.SUnit> opdestroyedUnits = new java.util.ArrayList<server.campaign.SUnit>();
                        if (unitsToDestroy > 0) {
                            ...
                            while (factoriesSearched.size() > 0) {
                                ...
                                for (int type = Unit.MEK; type < Unit.MAXBUILD; type++) {
                                    ...
                                    while (!noUnits && (numDestroyed < unitsToDestroy)) {
                                        server.campaign.SUnit destroyed = aLoser.getHouseFightingFor()
                            ...
                            if (numDestroyed >= 1) {
                                ...
                                // get the units
                                String unitString = "";
                                for (server.campaign.SUnit currU : opdestroyedUnits) {
                                    unitString += currU.getModelName() + ", ";
                                }
                                ...
                                while (!noPP && (ppDestroyed < ppToDestroy)) {
                                    ...
                                    if (ppAvailable <= 0) {
                                        ...
                                        if (toDestroy > 0) {
                                            ...
                                            loserHSUpdates.append(aLoser.getHouseFightingFor()
                                                                        .addPP(currWeight, type, -toDestroy, false));
                    ...
                    try {
                        ...
                        if ((so.getTargetWorld().getOwner() == null) ||
                                  !so.getTargetWorld()
                                         .getOwner()
                                         .getName()
                                         .equalsIgnoreCase(aWinner.getHouseFightingFor().getName())) {
                            totalRefreshBoost = 0;
                        }
                        ...
                        if (target.getFactoryCount() > 0) {
                            ...
                            if (ppToGenerate > 0) {
                                // get a random factory
                                server.campaign.SUnitFactory currFacility = target.getRandomUnitFactory();
                                ...
                                for (int type = Unit.MEK; type < Unit.MAXBUILD; type++) {
                                    ...
                                    winnerHSUpdates.append(aWinner.getHouseFightingFor()
                                                                 .addPP(currFacility.getWeightclass(),
                                                                       type,
                                                                       ppToGenerate,
                                                                       false));
                                    ...
                                    winnerMetaString += " generated an extra " +
                                                              ppToGenerate +
                                                              " " +
                                                              Unit.getWeightClassDesc(currFacility.getWeightclass()) +
                                                              " " +
                                                              Unit.getTypeClassDesc(type) +
                                                              " components";
     ...
     */
    private void possibleSalvageFromReport(java.util.StringTokenizer reportTokenizer, ShortOperation so) {

        try {// main catch, make new lists and a new pilots hash.

            salvagableUnits = new java.util.TreeMap<Integer, OperationEntity>();
            destroyedUnits = new java.util.TreeMap<Integer, OperationEntity>();
            ...
            try {
                ...
                while (reportTokenizer.hasMoreTokens()) {
                    ...
                    if (currentUnit.startsWith("MW*")) {
                        ...
                        java.util.StringTokenizer mwTokenizer = new java.util.StringTokenizer(currentUnit, "*");
    ...
    /**
     * Method which builds possible salvage from a Short Operation's death tree. Since the information stored while a
     * game is in progress is less exact than a full autoreport, some assumptions must be made. By setting up accurate
     * living, salvage and dead unit treemaps, this method allows the resolver to use its remaining methods (salvage
     * assignment, etc) as usual.
     */
    public void possibleSalvageFromInProgressInfo(ShortOperation so, server.campaign.SPlayer loser) {
        try {
             ...
             */
            java.util.ArrayList<OperationEntity> dropLivingUnits = new java.util.ArrayList<OperationEntity>();
            try {
    ...
    /**
     * Method which cleans up when a pilot dies. Determines whether or not peronal queues are in use, re-BV's the unit
     * and the armies in which it is resident, and returns a string detailing the outcome.
     */
    private String handleDeadPilot(server.campaign.SPlayer owner, server.campaign.SUnit unit, OperationEntity entity,
          ShortOperation op) {
        // holder string
        String toReturn = "";
    ...
    /**
     * Method which handles living, dispossessed pilots. Called from the salvage method. Very simple. Broken out to
     * slice repetetive code.
     */
    private void handleDispossesedPilot(server.campaign.SPlayer owner, server.campaign.SUnit unit) {
    ...
    /**
     * Method which checks the status of a pilot. Called when checking on salvaged and destroyed units. If the pilot is
     * non-null, can check the is dead field for status and reason for showing on field. If the pilot is null, death is
     * presumed. Can derive the cause of death from the status of his unit. This method returns an array which has very
     * specific object ordering, as follows: 0 - Boolean pilotIsDead; indicates status of pilot 1 - String ownerString;
     * to send to the original owner 2 - String captorString; to send to capturing player 3 - String otherString; to
     * send to player who are neither original owners nor captors 4 - SPlayer captor; player who has captured a pilot
     */
    private Object[] setupPilotStringForUnit(OperationEntity currEntity, server.campaign.SUnit currUnit,
        ...
        try {
            ...
            if (mw != null) {
                ...
                if (CampaignMain.campaignMain.getRandomNumber(100) < captureChance) {
                    ...
                    // lives, but we say
                    // that he is dead.
                    toReturn[2] = ((SPilot) currUnit.getPilot()).getPilotCaptureAndDefectedMessage(currUnit,
            ...
            if ((currEntity.getHDint() <= 0) &&
                ...
                return toReturn;
                              ...
                              (currEntity.getRemovalReason() == IEntityRemovalConditions.REMOVE_DEVASTATED)) &&
                             (currEntity.getCockpitType() == Mech.COCKPIT_TORSO_MOUNTED)) {
                toReturn[1] = currUnit.getPilot().getName() +
                                    " died when the " +
                                    currUnit.getModelName() +
            ...
            // didn't survive. can set up the message to owner and select a
            // random pickup player
            server.campaign.SPlayer pickupPlayer = selectRandomWinner();
            if (pickupPlayer == null) {
    ...
    /**
     * Method which returns the cost of salvaging a given unit. A unit costs less to salvage after a headkill, more to
     * salvage if torsos are destroyed, etc.
     */
    private int getSalvageCost(server.campaign.SPlayer owner, server.campaign.SUnit u, OperationEntity oe, Operation o,
          ShortOperation so) {
    ...
    /**
     * Method which determines how much, if any, compensation a faction will give a player after a unit is lost (to
     * either irreparable damage or an enemy) in a game. Flat/Percentage Caps and unit type adjustments allow admins to
     * give greater amounts (relatively) for smaller units, meks, and so on.
     */
     ...
     */
    private server.campaign.SPlayer selectRandomWinner() {
    ...
    /**
     * Method which selects a random player from the so.getLosers() tree. Used to assign salvage which is gained from
     * so.getWinners() (used rarely).
     */
    private server.campaign.SPlayer selectRandomLoser() {
    ...
    private String newPilotDescription(server.campaign.SUnit u) {
        String result = " ";
        try {
    ...
    private void contractFinishedInfo(server.campaign.SPlayer player) {
    ...
    private String calculatePilotEXP(Operation o, ShortOperation so, OperationEntity currEntity,
          server.campaign.SPlayer owner, boolean allowLevelUp) {
        ...
        if ((currEntity.getKills().size() > 0) && countKills) {
            ...
            for (Integer id : currEntity.getKills()) {
                ...
                // unit or mechwarrior.
                server.campaign.SUnit killedUnit = currEntity.getOwner().getUnit(currEntity.getID());
                if (killedUnit == null) {
                    continue;
                }
    ...
    /**
     * @salient - added for mini campaigns If enabled, this will check SO's for appropriate currency injection if
    ...
    private void repodUnits(ShortOperation so, Operation o) {
        ...
        for (String player : so.getAllPlayersAndArmies().keySet()) {
            ...
            server.campaign.SArmy curra = currp.getArmy(so.getAllPlayersAndArmies().get(player));

            try {
                java.util.Enumeration<Unit> units = curra.getUnits().elements();
                while (units.hasMoreElements()) {
                    server.campaign.SUnit unit = (server.campaign.SUnit) units.nextElement();
                    ...
                    for (House house : CampaignMain.campaignMain.getData().getAllHouses()) {
                        ...
                        buildFile = server.campaign.BuildTable.getFileName("Common",
                              Unit.getWeightClassDesc(unit.getWeightclass()),
                              timeZone,
                              unit.getType());
                        if (!tables.contains(buildFile)) {
                    ...
                    for (String buildTable : tables) {
                        ...
                        try {
                            ...
                            java.io.FileInputStream fis = new java.io.FileInputStream(buildTable);
                            ...
                            while (dis.ready()) {
                                ...
                                if (ST.hasMoreElements()) {
                                    ...
                                    if (unitFile.toLowerCase().startsWith(repodChassie.toLowerCase())) {
                                        ...
                                        server.campaign.SUnit cm = new server.campaign.SUnit(unit.getId(),
                                              unit.getProducer(),
                                              unitFile);
