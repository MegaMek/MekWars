...
public class ShortValidator {
     ...
     * may be called with 2 different goals:
     * <p>
     * 1) "Real" validation compares the potential attacker with possible defenders. This call is used when a player
     * sends a /c attack command with a specific ShortOp type intended.
     * <p>
     * 2) "Dry" validations look simply at the qualities of an attacking army to see whether or not it is valid for a
     * given operation type. Players can trigger this run-mode by calling by /c eligibility. It's also used to check
     * armies as players add/remove units.
     *
     ...
     *
     * @return - an ArrayList with reasons the player (basic checks) or army (advanced checks) is not eligible to
     *       participate in the operation.
     */
    public java.util.ArrayList<Integer> validateShortAttacker(server.campaign.SPlayer ap, server.campaign.SArmy aa,
          Operation o, server.campaign.SPlanet target, int longID, boolean joiningAttack) {
         ...
         */
            ...
            if (aa.getOpponents() == null ||
                      aa.getOpponents().size() == 0 && !o.getBooleanValue("AttackerAllowAgainstUnclaimedLand")) {
            ...
            java.util.ArrayList<server.campaign.SArmy> planetMatches = new java.util.ArrayList<server.campaign.SArmy>();
            java.util.ArrayList<server.campaign.SArmy> fullMatches = new java.util.ArrayList<server.campaign.SArmy>();

            java.util.Iterator<server.campaign.SArmy> i = aa.getOpponents().iterator();
            while (i.hasNext()) {
                ...
                server.campaign.SHouse currHouse = currPlayer.getHouseFightingFor();

                // check the currHouse's planetary holdings
                if (o.getBooleanValue("FreeForAllOperation") ||
                          target.getInfluence().getInfluence(currHouse.getId()) > 0) {planetMatches.add(currArmy);}
            }
            ...
            if (o.getBooleanValue("AttackerAllowAgainstUnclaimedLand") && target.getInfluence().getInfluence(-1) > 0) {
                ...
                ShortOperation newOp = new ShortOperation(o.getName(),
                      target,
                      ap,
                      aa,
                      fullMatches,
                ...
                if (conquestBVAdjust > 0) {totalConquest += Math.floor(newOp.getStartingBV() / conquestBVAdjust);}
                ...
                if (totalConquest > 0) {
                    ...
                    String winnerMetaString = " gained " +
                                                    totalConquest +
                                                    " " +
                                                    point +
                                                    " of " +
                    ...
                    if (!CampaignMain.campaignMain.getBooleanConfig("ShowCompleteGameInfoInNews")) {
                        ...
                        newsFeedBody = ap.getHouseFightingFor().getColoredNameAsLink() +
                                             " gained " +
                                             totalConquest +
                                             "cp on " +
                                             newOp.getTargetWorld().getName();
                    ...
                    newOp.setCompleteFinishedInfo(ap.getName() +
                                                        ...
                                                        "cp on " +
                                                        newOp.getTargetWorld().getName());
                    newOp.setIncompleteFinishedInfo(ap.getHouseFightingFor().getColoredNameAsLink() +
                                                          " gained " +
                                                          totalConquest +
                                                          "cp on " +
                                                          newOp.getTargetWorld().getName());
            ...
            for (server.campaign.SArmy currArmy : planetMatches) {
                ...
                if (maxELO > 0 && Math.abs(currPlayer.getRating() - ap.getRating()) > maxELO) {
                    defenderFails.add(mekwars.server.campaign.operations.ShortValidator.SFAIL_COMMON_ELODIFFERENCE);
                }
                if (!aa.matches(currArmy, o)) {
                    defenderFails.add(mekwars.server.campaign.operations.ShortValidator.SFAIL_COMMON_MAX_BV_DIFFERENCE);
                }
     ...
     */
    public void checkAttackerMilestones(java.util.ArrayList<Integer> failureReasons, server.campaign.SPlayer ap,
          Operation o) {

        boolean solCanAttack = o.getBooleanValue("AllowSOLToUse");
        boolean nonConqCanAttack = o.getBooleanValue("AllowNonConqToUse");
        if (ap.getHouseFightingFor().isNewbieHouse() && !solCanAttack) {failureReasons.add(SFAIL_ATTACK_SOLCANTATT);}
        ...
        if (ap.getRating() > o.getDoubleValue("MaxAttackerRating")) {failureReasons.add(SFAIL_ATTACK_MAXRATING);}
        if (ap.getRating() < o.getDoubleValue("MinAttackerRating")) {failureReasons.add(SFAIL_ATTACK_MINRATING);}

        if (ap.getExperience() > o.getIntValue("MaxAttackerXP")) {failureReasons.add(SFAIL_ATTACK_MAXXP);}
        if (ap.getExperience() < o.getIntValue("MinAttackerXP")) {failureReasons.add(SFAIL_ATTACK_MINXP);}
    ...
    /**
     * Method which checks army CONSTRUCTION params (contruction block of the defaults). These dictate simple things
     * like the max aggregate weight of all units in the army and are specific to the given army, not relative to the
     * defending force.
     * <p>
     * Checks the following Operation properties: - MaxAttackerBV - MinAttackerBV - MaxAttackerUnits - MinAttackerUnits
     * - MinAttackerWalk - MinAttackerJump - MaxAttackerUnitTonnage - MinAttackerUnitTonnage - MaxTotalAttackerTonnage -
     * MinTotalAttackerTonnage - AttackerAllowedMeks - AttackerAllowedVehs - AttackerAllowedInf - AttackerOmniMeksOnly -
     * PoweredInfAllowed, if !AttackerAllowedInf
     */
    public void checkAttackerConstruction(java.util.ArrayList<Integer> failureReasons, server.campaign.SArmy aa,
          Operation o) {
        ...
        if (o.getBooleanValue("AttackerUsePercentageBVSpread")) {
            isv = new PercentBVSpreadValidator(o.getIntValue("MaxAttackerUnitBVSpread"),
                  o.getDoubleValue("AttackerBVSpreadPercent"));
        } else {
            isv = new StandardBVSpreadValidator(o.getIntValue("MinAttackerUnitBVSpread"),
        ...
        // matching.
        if (aa.getBV() > o.getIntValue("MaxAttackerBV")) {failureReasons.add(SFAIL_ATTACK_MAXBV);} else if (aa.getBV() <
                                                                                                                  o.getIntValue(
                                                                                                                        "MinAttackerBV")) {
        ...
        } else if (infCount < o.getIntValue("MinAttackerInfantry")) {failureReasons.add(SFAIL_ATTACK_MININFANTRY);}
        ...
        if (aa.getNumberOfUnitTypes(Unit.MEK) +
                  ...
                  aa.getNumberOfUnitTypes(Unit.AERO) > o.getIntValue("MaxAttackerNonInfantry")) {
            failureReasons.add(SFAIL_ATTACK_MAXNONINFANTRY);
        } else if (aa.getNumberOfUnitTypes(Unit.MEK) +
                         aa.getNumberOfUnitTypes(Unit.VEHICLE) +
                         aa.getNumberOfUnitTypes(Unit.AERO) < o.getIntValue("MinAttackerNonInfantry")) {
        ...
        java.util.Iterator<Unit> i = aa.getUnits().iterator();
        while (i.hasNext()) {
            ...
            if (currUnit.getType() == Unit.MEK ||
                ...
                if (currWeight > o.getIntValue("MaxAttackerUnitTonnage")) {maxTonFail = true;} else if (currWeight <
                                                                                                              o.getIntValue(
            ...
            if (currBV < o.getIntValue("MinAttackerUnitBV")) {minBVFail = true;} else if (currBV >
                                                                                                o.getIntValue(
                                                                                                      "MaxAttackerUnitBV")) {
                maxBVFail = true;
            }
            ...
            if (!currUnit.hasVacantPilot()) {
                ...
                if (totalSkills > o.getIntValue("HighestAttackerPilotSkillTotal")) {greenPilots = true;}
        ...
        if (hasMeks && !o.getBooleanValue("AttackerAllowedMeks")) {failureReasons.add(SFAIL_ATTACK_NOMEKS);}
        if (hasVehs && !o.getBooleanValue("AttackerAllowedVehs")) {failureReasons.add(SFAIL_ATTACK_NOVEHS);}
        if (hasAeros && !o.getBooleanValue("AttackerAllowedAeros")) {failureReasons.add(SFAIL_ATTACK_NOAEROS);}
     ...
     * <p>
     * Unlike Attack validations, Defenses do NOT have a dry call. Every check looks at volatiles.
     *
     * @param - SPlayer dp, defending player
     * @param - SPlayer da, defending army
     * @param - Operation o, operation type/settings
     */
    public java.util.ArrayList<Integer> validateShortDefender(server.campaign.SPlayer dp, server.campaign.SArmy da,
