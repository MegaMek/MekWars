...
public class OpsChickenThread extends Thread {
    ...
    private java.util.Vector<server.campaign.SArmy> parmies;
    private int opID;
    ...
    // CONSTRUCTOS
    public OpsChickenThread(server.campaign.SPlayer pd, int id, String name, String mess) {
        pdefender = pd;
        parmies = new java.util.Vector<server.campaign.SArmy>(1, 1);
        shouldContinue = true;
        //leechCount = 0;
        message = mess;
        opID = id;
    ...
    // METHODS
    public void addArmy(server.campaign.SArmy a) {
        parmies.add(a);
    }

    public java.util.Vector<server.campaign.SArmy> getArmies() {
        return parmies;
    }
     ...
     * <p>
     * Note - the thread will not take note of the change until it next wakes. At that point it will terminate. This
    ...
    public synchronized void doPenalty() {
        ...
        if (totalEXPLoss > 0) {
            if (!hasLoss) {toPlayer += " (-" + totalEXPLoss + " XP";} else {toPlayer += ", -" + totalEXPLoss + " XP";}
        ...
        server.campaign.SPlanet target = parentOp.getTargetWorld();

        String toMain = pdefender.getColoredName() +
                              " did not defend " +
        ...
        if (unitPenalty > 0) {
            ...
            for (int type = Unit.MEK; type < Unit.MAXBUILD; type++) {
                ...
                for (int weight = Unit.ASSAULT; weight >= Unit.LIGHT; weight--) {
                    ...
                    while (!noUnits && numCaptured < unitPenalty) {
                        server.campaign.SUnit captured = defendH.getEntity(weight, type);
                        if (captured == null) {noUnits = true;} else {
            ...
            if (numCaptured > 0) {
                ...
                if (hasLoss) {
                    // toMain += ", " + numCaptured;
                    if (numCaptured == 1) {toMain += ", a unit [" + unitString + "]";} else {
