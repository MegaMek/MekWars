 ...
 * updated while the auction is running. All other info is passed at construction time and may not be changed while the
 * auction is running.
 *
 * @author urgru
 */
public final class MarketListing {
    ...
    private java.util.TreeMap<String, mekwars.server.campaign.market2.MarketBid> bidsReceived;
    ...
    //CONSTRUCTOR
    public MarketListing(String sellname, server.campaign.SUnit unit, int minbid, int duration) {
        sellerName = sellname;
        listedUnitID = unit.getId();
        minimumBid = minbid;
        saleDuration = duration;
        bidsReceived = new java.util.TreeMap<String, mekwars.server.campaign.market2.MarketBid>();
     ...
     */
    public String toString(int id, server.campaign.SPlayer p) {
     ...
     */
    public java.util.TreeMap<String, mekwars.server.campaign.market2.MarketBid> getAllBids() {
        return bidsReceived;
    }

    /**
     * Return the bid from a given player. Used by Market2.getAutoMarketStatus() and RecallBidCommand.java.
     */
    public int getBidForPlayer(server.campaign.SPlayer p) {
    ...
    /**
     * Return the ID# of the unit being sold. Note that the unit itself is NOT stored anywhere within the market, and
     * must be accessed by using SPlayer's getUnit(int unitID) or equivalent method in SHouse.
     */
    ...
    /**
     * Returns the simulated model name (i.e., "Heavy Mek") for an auction where the units are hidden
     */
    ...
    /**
     * Decrease the remaining sale-ticks by one. This is called by Market.java during a tick. Don't worry about negative
     * values here, as the Market will resolve any auction with a duration of less than 1 before decrementing.
     */
    ...
    /**
     * This is called for units that are placed on the market at the tick due to the tick decrementing the sale after
     * its listed causing units to appear with -1 ticks left.
     *
     */
