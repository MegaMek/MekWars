...
/**
 * Vickrey auction is a modified highest sealed bid auction. Winner determination is the same (highest bid, earliest
 * placement in the event of a tie), but the winner pays 2nd highest bid, plus one, in lieu of the amount he offered.
 * <p>
 * If only one player bids, he pays full fare. If no one bids, winner is "SERVER" and the Market destroys the unit.
 *
 * @author urgru
 */
public final class VickreyAuction implements IAuction {
     ...
     * <p>
     * - If several people bid, winner pays next-highest + 1. - In only one person bids, winner pays the minimum.
     */
    public MarketBid getWinner(MarketListing listing, boolean hiddenBM) {
         ...
         */
        java.util.TreeMap<mekwars.server.campaign.market2.MarketBid, String> orderedBids = new java.util.TreeMap<mekwars.server.campaign.market2.MarketBid, String>();
        java.util.TreeMap<String, mekwars.server.campaign.market2.MarketBid> placedBids = listing.getAllBids();
         ...
         */
        java.util.Iterator<mekwars.server.campaign.market2.MarketBid> i = orderedBids.keySet().iterator();
        while (i.hasNext()) {
        ...
        if (i.hasNext()) {
            ...
            IBuyer potentialWinner = server.campaign.CampaignMain.cm.getPlayer(nextBid.getBidderName());
            if (potentialWinner == null) {
                potentialWinner = server.campaign.CampaignMain.cm.getHouseFromPartialString(nextBid.getBidderName(),
                      null);
            }
