...
public class SHouse extends TimeUpdateHouse implements Comparable<Object>, ISeller, IBuyer, Serializable {
    private static final MMLogger LOGGER = MMLogger.create(SHouse.class);
    ...
    public String fromString(String string, Random random) {
        try {
            ...
            LOGGER.info("House loaded: " + getName());
    ...
    /**
     * Returns the number of players who count for mintick production. Called from SHouse.tick(). Factored out to keep
     * the tick more or less redable.
     * <p>
     * An active player counts if he has at least one army that is between min and max BVs to count, and has been active
     * for a whole tick.
     * <p>
     * Fighting players may or may not count, depending on the weight assigned by the admins to the game-type they are
     * playing.
     *
    ...
    /**
     * have the faction perform tick duties (gather income, referesh factories) and clean out its hangars and PP
     * excesses (either via scrapping, industrial accidents, or BM sales), then report the tick results to all of its
     * faction members.
     */
    public String tick(boolean real, int tickid) {
        ...
        for (int type_id = 0; type_id < Unit.TOTALTYPES; type_id++) {
            for (int i = Unit.LIGHT; i <= Unit.ASSAULT; i++) {
                ...
                while (remainingHangarSpaceForWeightclass(i, type_id) < 0) {
                    ...
                    java.util.Vector<mekwars.server.campaign.SUnit> v = this.getHangar(type_id).elementAt(i);
            ...
            for (int weight = 0; weight < 4; weight++) {
                ...
                while (getPP(weight, type_id) > getMaxAllowedPP(weight, type_id)) {
                    ...
                    if (Boolean.parseBoolean(this.getConfig("UseAutoProdClassic"))) {
                        ...
                        failureRateToUse = Integer.parseInt(this.getConfig("APFailureRate" +
                    ...
                    // else, make a new unit
                        ...
                        java.util.Vector<mekwars.server.campaign.SUnit> newUnits = m.getMechProduced(type_id,
                              getNewPilot(type_id));
                        for (SUnit newUnit : newUnits) {
                            ...
                            if (!Boolean.parseBoolean(this.getConfig("UseCalculatedCosts"))) {
                                ...
                                if (m.getWeightclass() == Unit.LIGHT) {
                                    hsUpdates.append(m.addRefresh((Integer.parseInt(this.getConfig("LightRefresh")) *
                                                                         100) / m.getRefreshSpeed(), false));
                                } else if (m.getWeightclass() == Unit.MEDIUM) {
                                    hsUpdates.append(m.addRefresh((Integer.parseInt(this.getConfig("MediumRefresh")) *
                                                                         100) / m.getRefreshSpeed(), false));
                                } else if (m.getWeightclass() == Unit.HEAVY) {
                                    hsUpdates.append(m.addRefresh((Integer.parseInt(this.getConfig("HeavyRefresh")) *
                                                                         100) / m.getRefreshSpeed(), false));
                                } else if (m.getWeightclass() == Unit.ASSAULT) {
                                    hsUpdates.append(m.addRefresh((Integer.parseInt(this.getConfig("AssaultRefresh")) *
                                                                         100) / m.getRefreshSpeed(), false));
                                }
                            }
                            ...
                            if (type_id == Unit.INFANTRY) {
                                // exclusive message
                                mechsProduced.append("A militia unit [" +
        ...
        if ((getShowProductionCountNext() - 1) <= 0) {
            ...
            if (Boolean.parseBoolean(this.getConfig("UseMek"))) {
                result += myFormatter.format(MekComponents / (Double.parseDouble(this.getConfig("LightPP")))) +
                                " Light meks<br>";
                result += myFormatter.format(MekComponents / (Double.parseDouble(this.getConfig("MediumPP")))) +
                                " Medium meks<br>";
                result += myFormatter.format(MekComponents / (Double.parseDouble(this.getConfig("HeavyPP")))) +
                                " Heavy meks<br>";
                result += myFormatter.format(MekComponents / (Double.parseDouble(this.getConfig("AssaultPP")))) +
                                " Assault meks<br>";
            }
            if (Boolean.parseBoolean(this.getConfig("UseVehicle"))) {
                result += myFormatter.format(VehComponents / (Double.parseDouble(this.getConfig("LightVehiclePP")))) +
                                " Light vehicles<br>";
                result += myFormatter.format(VehComponents / (Double.parseDouble(this.getConfig("MediumVehiclePP")))) +
                                " Medium vehicles<br>";
                result += myFormatter.format(VehComponents / (Double.parseDouble(this.getConfig("HeavyVehiclePP")))) +
                                " Heavy vehicles<br>";
                result += myFormatter.format(VehComponents / (Double.parseDouble(this.getConfig("AssaultVehiclePP")))) +
                                " Assault vehicles<br>";
            }
            if (Boolean.parseBoolean(this.getConfig("UseInfantry"))) {
                ...
                if (Boolean.parseBoolean(this.getConfig("UseOnlyLightInfantry"))) {
                    result += myFormatter.format(InfComponents /
                                                       (Double.parseDouble(this.getConfig("LightInfantryPP")))) +
                                    " Infantry<br>";
                } else {
                    result += myFormatter.format(InfComponents /
                                                       (Double.parseDouble(this.getConfig("LightInfantryPP")))) +
                                    " Light infantry<br>";
                    result += myFormatter.format(InfComponents /
                                                       (Double.parseDouble(this.getConfig("MediumInfantryPP")))) +
                                    " Medium infantry<br>";
                    result += myFormatter.format(InfComponents /
                                                       (Double.parseDouble(this.getConfig("HeavyInfantryPP")))) +
                                    " Heavy infantry<br>";
                    result += myFormatter.format(InfComponents /
                                                       (Double.parseDouble(this.getConfig("AssaultInfantryPP")))) +
                                    " Assault infantry<br>";
                }
            }// end if(UseInfantry)
            if (Boolean.parseBoolean(this.getConfig("UseProtoMek"))) {
                result += myFormatter.format(ProtoComponents /
                                                   (Double.parseDouble(this.getConfig("LightProtoMekPP")))) +
                                " Light protomechs<br>";
                result += myFormatter.format(ProtoComponents /
                                                   (Double.parseDouble(this.getConfig("MediumProtoMekPP")))) +
                                " Medium protomechs<br>";
                result += myFormatter.format(ProtoComponents /
                                                   (Double.parseDouble(this.getConfig("HeavyProtoMekPP")))) +
                                " Heavy protomechs<br>";
                result += myFormatter.format(ProtoComponents /
                                                   (Double.parseDouble(this.getConfig("AssaultProtoMekPP")))) +
                                " Assault protomechs<br>";
            }
            if (Boolean.parseBoolean(this.getConfig("UseBattleArmor"))) {
                result += myFormatter.format(BAComponents /
                                                   (Double.parseDouble(this.getConfig("LightBattleArmorPP")))) +
                                " Light battle armor<br>";
                result += myFormatter.format(BAComponents /
                                                   (Double.parseDouble(this.getConfig("MediumBattleArmorPP")))) +
                                " Medium battle armor<br>";
                result += myFormatter.format(BAComponents /
                                                   (Double.parseDouble(this.getConfig("HeavyBattleArmorPP")))) +
                                " Heavy battle armor<br>";
                result += myFormatter.format(BAComponents /
                                                   (Double.parseDouble(this.getConfig("AssaultBattleArmorPP")))) +
                                " Assault battle armor<br>";
            }

            if (Boolean.parseBoolean(this.getConfig("UseAero"))) {
                result += myFormatter.format(AeroComponents / (Double.parseDouble(this.getConfig("LightAeroPP")))) +
                                " Light aero<br>";
                result += myFormatter.format(AeroComponents / (Double.parseDouble(this.getConfig("MediumAeroPP")))) +
                                " Medium aero<br>";
                result += myFormatter.format(AeroComponents / (Double.parseDouble(this.getConfig("HeavyAeroPP")))) +
                                " Heavy aero<br>";
                result += myFormatter.format(AeroComponents / (Double.parseDouble(this.getConfig("AssaultAeroPP")))) +
                                " Assault aero<br>";
            }
    ...
    /**
     * Helper method for SHouse.getExcuseForUnitFailure() that factors out highly redundant input stream code.
     */
     ...
     * randomly on ticks or pursuant to a general purchase request are from the faction's own tables.
     */
    public SUnitFactory getNativeFactoryForProduction(int type, int weight, boolean useOnlyOriginalFactories) {
        ...
        java.util.Vector<SUnitFactory> allPossible = getPossibleFactoryForProduction(type, weight, false);
        if (allPossible.size() == 0) {
            return null;
        }

        // sort out non-faction factories and return if none exist
        java.util.Vector<SUnitFactory> factionPossible = new java.util.Vector<SUnitFactory>(1, 1);
    ...
    /**
     * @Salient , this is for subfaction enforcement rule when player presses buy new, it pulls from correct
     *       factory
     *
     */
    public SUnitFactory getNativeAccessableFactoryForProduction(int type, int weight, int subFactionLvl,
          String Username) {
        ...
        java.util.Vector<SUnitFactory> accessPossible = new java.util.Vector<SUnitFactory>(1, 1);
    ...
    public void calcActivityPP(Double armyWeight) {
        ...
        java.util.Iterator<SPlanet> e = getPlanets().values().iterator();
    ...
    /**
     * A method which adds a specified number of PP to Stores of the given weight class. Can send house status updates,
     * but also returns cmd to be added to longer lists of changes.
     *
    ...
    public String addPP(int weight, int type_id, int val, boolean sendUpdate) {
        ...
        try {
            ...
            java.util.Vector<Integer> v = new java.util.Vector<Integer>(4, 1);
            for (int i = 0; i < 4; i++) {
                // Weight
                v.add(0);
            }
    ...
    /**
     * A method which returns a unit from the SHouse's queue. This should only be called from SHouse (during ticks) or
     * RequestDonatedCommand (during an ask). If there is no queue'd unit of the given weightclass/type, a null is
     * returned.
     * <p>
     * WARNING!! getEntity() returns a unit, which means it cannot return a HS| command string like removeUnit() does.
     * Code that makes use of getEntity will need to set up and send one using getHSUnitRemovalString().
     */
    public SUnit getEntity(int weightclass, int type_id) {
        java.util.Vector<mekwars.server.campaign.SUnit> s;
        try {
        ...
        if (getNumberOfNonSaleUnits(s) > 0) {
            ...
            java.util.Vector<mekwars.server.campaign.SUnit> unitsToBuy = new java.util.Vector<mekwars.server.campaign.SUnit>(
                  s.size(),
                  1);
    ...
    private int getNumberOfNonSaleUnits(java.util.Vector<mekwars.server.campaign.SUnit> units) {
        int count = 0;
    ...
    /**
     * Simple method which determines whether a given SHouse (and its players) may access the market to SELL units. We
     * check this loop continuously instead of saving a value in the SHouse (inefficient) b/c the config may change
     * between checks.
     */
    public boolean maySellOnBM() {
        java.util.StringTokenizer blockedFactions = new java.util.StringTokenizer(this.getConfig("BMNoSell"), "$");
    ...
    /**
     * Simple method which determines whether a given SHouse (and its players) may access the market to BUY units. We
     * check this loop continuously instead of saving a value in the SHouse (inefficient) b/c the config may change
     * between checks.
     */
    public boolean mayBuyFromBM() {
        java.util.StringTokenizer blockedFactions = new java.util.StringTokenizer(this.getConfig("BMNoBuy"), "$");
        while (blockedFactions.hasMoreTokens()) {
            if (getName().equals(blockedFactions.nextToken())) {
                return false;
            }
    ...
    public String removeUnit(SUnit unitToRemove, boolean sendUpdate) {
        ...
        java.util.Vector<mekwars.server.campaign.SUnit> Weightclass = this.getHangar(unitToRemove.getType())
                                                                            .elementAt(unitToRemove.getWeightclass());
    ...
    protected String doLogin(SPlayer p) {
        ...
        if (smallp == null) {// make a new one
            smallp = new SmallPlayer(p.getExperience(),
                  p.getLastOnline(),
                  p.getRating(),
                  realName,
                  p.getFluffText(),
    ...
    /**
     * Generates serialized version of SHouse to send to clients for HouseStatus tab. Complete status is sent on login.
     * Afterwards, changes are transmitted incremementally.
     */
        ...
        if (!(this.isNewbieHouse() && Boolean.parseBoolean(CampaignMain.campaignMain.getConfig("HiddenBMUnits")))) {
            for (int type_id = 0; type_id < Unit.TOTALTYPES; type_id++) {
                ...
                for (int weight = Unit.LIGHT; weight <= Unit.ASSAULT; weight++) {
                    ...
                    java.util.Vector<mekwars.server.campaign.SUnit> unitSet = this.getHangar(type_id).elementAt(
                          weight);
    ...
    /**
     * Construct a string to send to clients if PP changes. Format is CC|weight$type$components$producableunits|
     */
    ...
    public int compareTo(Object o) {
        mekwars.server.campaign.SHouse h = (mekwars.server.campaign.SHouse) o;
     ...
     */
    public java.util.Hashtable<String, SPlayer> getAllOnlinePlayers() {
        java.util.Hashtable<String, SPlayer> allPlayers = new java.util.Hashtable<String, SPlayer>();
        ...
        if (getComponentConverter().containsKey("All")) {
            ...
            for (BMEquipment eq : CampaignMain.campaignMain.getPartsMarket().getEquipmentList().values()) {
                ...
                if (getPartsAmount(eq.getEquipmentInternalName()) < minCrits) {
                    ...
                    if (getComponents().get(converter.getComponentUsedType()).get(converter.getComponentUsedWeight()) <
                              components) {
                        continue;
                    }

                    addPP(converter.getComponentUsedWeight(),
