...
public class UnitCosts {
    private static final MMLogger LOGGER = MMLogger.create(UnitCosts.class);

    java.util.Vector<java.util.Vector<Double>> minCostUnitList = new java.util.Vector<java.util.Vector<Double>>(4, 1);
    java.util.Vector<java.util.Vector<Double>> maxCostUnitList = new java.util.Vector<java.util.Vector<Double>>(4, 1);

    public UnitCosts() {
        for (int weight = Unit.LIGHT; weight <= Unit.ASSAULT; weight++) {
            java.util.Vector<Double> maxVector = new java.util.Vector<Double>(4, 1);
            java.util.Vector<Double> minVector = new java.util.Vector<Double>(4, 1);
    ...
    public void loadUnitCosts() {
        ...
        try {
            java.io.FileInputStream in = new java.io.FileInputStream("./data/mechfiles/Meks.zip");
            java.util.zip.ZipInputStream zipFile = new java.util.zip.ZipInputStream(in);

            while (zipFile.available() == 1) {

                java.util.zip.ZipEntry entry = zipFile.getNextEntry();
        ...
        try {
            java.io.FileInputStream in = new java.io.FileInputStream("./data/mechfiles/Vehicles.zip");
            java.util.zip.ZipInputStream zipFile = new java.util.zip.ZipInputStream(in);
        ...
        try {
            java.io.FileInputStream in = new java.io.FileInputStream("./data/mechfiles/Infantry.zip");
            java.util.zip.ZipInputStream zipFile = new java.util.zip.ZipInputStream(in);
    ...
    public String displayUnitCostsLists() {
        ...
        for (int weight = 0; weight <= Unit.ASSAULT; weight++) {
            for (int type = 0; type < Unit.MAXBUILD; type++) {
                result.append(Unit.getWeightClassDesc(weight) +
                                    " " +
